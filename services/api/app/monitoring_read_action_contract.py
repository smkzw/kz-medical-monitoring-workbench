...
class _R5SurfaceName(str):
    """String-compatible R5 name kept out of the legacy enum iteration set."""
...
R5_OVERVIEW = _R5SurfaceName("r5_overview")
R5_SUBJECT_WORKSPACE = _R5SurfaceName("r5_subject_workspace")
R5_SOURCE_EVIDENCE = _R5SurfaceName("r5_source_evidence")
R5_READ_SURFACES = frozenset(
    {R5_OVERVIEW.value, R5_SUBJECT_WORKSPACE.value, R5_SOURCE_EVIDENCE.value}
)
# Expose discoverable names without changing the legacy enum's iteration set;
# existing consumers use that iteration as their compatibility contract.
MonitoringReadSurface.R5_OVERVIEW = R5_OVERVIEW  # type: ignore[attr-defined]
MonitoringReadSurface.R5_SUBJECT_WORKSPACE = R5_SUBJECT_WORKSPACE  # type: ignore[attr-defined]
MonitoringReadSurface.R5_SOURCE_EVIDENCE = R5_SOURCE_EVIDENCE  # type: ignore[attr-defined]


def _surface_value(surface: Any) -> str:
    return str(getattr(surface, "value", surface))
...
_R5_SURFACE_ACTION: dict[str, MonitoringAction] = {
    R5_OVERVIEW.value: MonitoringAction.READ_MONITORING,
    R5_SUBJECT_WORKSPACE.value: MonitoringAction.READ_MONITORING,
    R5_SOURCE_EVIDENCE.value: MonitoringAction.READ_SOURCE_EVIDENCE,
}


def _surface_action(surface: Any) -> MonitoringAction:
    if isinstance(surface, MonitoringReadSurface):
        return _SURFACE_ACTION[surface]
    return _R5_SURFACE_ACTION[_surface_value(surface)]
...
class MonitoringReadActionContract:
    ...
    surface: MonitoringReadSurface | str
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.surface, MonitoringReadSurface):
            surface_value = _surface_value(self.surface)
            if surface_value in R5_READ_SURFACES:
                object.__setattr__(self, "surface", _R5SurfaceName(surface_value))
            else:
                try:
                    object.__setattr__(
                        self, "surface", MonitoringReadSurface(surface_value)
                    )
        ...
        surface_value = _surface_value(self.surface)
        if surface_value == R5_OVERVIEW.value:
            # Project overview remains project-targeted; a site-scoped
            # overview binds the exact site target and never falls back.
            if audit.target_type == "project":
                valid_target = audit.target_id == route.project_id
            elif audit.target_type == "site":
                valid_target = bool(audit.target_id)
            else:
                valid_target = False
            if not valid_target:
                raise MonitoringReadActionContractError(
                    "R5 overview requires a project or exact site audit target"
                )
        elif self.surface in _PROJECT_TARGET_SURFACES or surface_value in _PROJECT_TARGET_SURFACES:
            if audit.target_type != "project" or audit.target_id != route.project_id:
                raise MonitoringReadActionContractError(
                    "project read surface requires a project audit target"
                )
        elif surface_value == R5_SUBJECT_WORKSPACE.value:
            if audit.target_type != "subject" or not audit.target_id:
                raise MonitoringReadActionContractError(
                    "subject read surface requires a subject audit target"
                )
        elif surface_value == R5_SOURCE_EVIDENCE.value:
            if audit.target_type != "source" or not audit.target_id:
                raise MonitoringReadActionContractError(
                    "source read surface requires a source audit target"
                )
    ...
    def _hash_payload(self) -> dict[str, Any]:
        return {
            ...
            "surface": _surface_value(self.surface),
