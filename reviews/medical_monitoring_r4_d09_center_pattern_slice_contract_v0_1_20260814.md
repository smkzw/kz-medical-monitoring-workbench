...
> 状态：`SELF_CONTAINED_FREEZE_CANDIDATE`  
> 日期：2026-08-14  
...
稳定核心不含 run ID、快照 ID、计算日期、mtime、进程 ID、数组顺序或显示文案。数据修订保持 stable core，并产生数据 lineage；规则、mapping、窗口、分层或算法变化产生新 unit identity，并以 `superseded_by_rule_or_method_change` 连接，禁止伪装为 `resolved_by_data`。
...
D09 public identity 明确使用：`project_ref / domain_id=D09 / scope_type=site / site_stable_id / stable_source_or_event_identity=(pattern_definition_id, analysis_window_stable_id, stratum_key) / normalized_concept=(pattern_kind, pattern_definition_id) / temporal_window=(window_kind, window_definition_id) / rule_or_knowledge_lineage / public_identity_version=d09_public_v1 / scope_binding_id / cutoff` 的 canonical tuple/hash，且永不与 D01-D08 public identity 合并。`unit_id` 不进入 public stable hash。
...
`D09HotspotProjection` 不是 L1 unit 或 RiskInstance。它按已冻结优先级规则列出中心内高风险或多域受试者，并保留：成员风险、绝对量、优先级、时间窗、coverage 和来源。任何 negative/小样本/中心低比例均不得隐藏高风险成员。
...
固定中文：`相关个体风险 N 条`、`受影响受试者 N 名`、`事件 N 起`、`中心模式 N 项`；disposition 显示 `发现问题`、`未发现问题`、`边界情况`、`不适用`、`暂无法评价（附原因）`；生命周期显示 `进行中`、`已核实关闭`、`已由规则变更取代`。中心模式显示 `clinical_label_zh`，不得暴露枚举。
...
全部分区互斥主归属，case 不得跨分区复用。冻结 catalog 下限为各行最小值之和：当前 **179 cases**。最终 case 数只增不减。
...
本文件仍为会商草案，不是冻结合同。后续必须由原参与会话定向确认上述问题已闭合，再由 Codex 生成自包含冻结候选并组织独立冻结验收。
