...
const bundlePath = path.join(here, ".r7-product-loop-render-bundle.cjs");
buildSync({
  entryPoints: [path.join(here, "MedicalMonitoringR7ProductLoop.jsx")],
  bundle: true,
  format: "cjs",
  platform: "node",
  outfile: bundlePath,
...
const noRunHtml = render(element("R7ProductWorkbar", { state: noRun, onAction: () => {} }));
...
const activeHtml = render(element("R7ProductWorkbar", { state: inFlight, onAction: () => {} }));
...
const publishedHtml = render(element("R7ProductWorkbar", { state: published, onAction: () => {} }));
...
const wizardHtml = render(element("R7WizardView", { wizard, onClose: () => {}, onSelect: () => {}, onAdvance: () => {}, onPreview: () => {}, onClosePreview: () => {}, onConfirmPreview: () => {} }));
check((wizardHtml.match(/r7-mode-card(?:\s|")/g) || []).length === 3, "wizard step one renders exactly three server modes");
...
const scopeWizardHtml = render(element("R7WizardView", {
  wizard: {
    ...wizard,
    step: 2,
...
const previewWizardHtml = render(element("R7WizardView", {
  wizard: {
    ...wizard,
    step: 3,
...
const confirmWizardHtml = render(element("R7WizardView", {
...
const historyHtml = render(element("R7HistoryDrawer", { history: { rows: [{ publicRunToken: "run:1", modeText: "日常监查", dataCutoffText: "2026-08-20", comparisonRangeText: "当前完整数据", statusText: "本次监查已完成", resultAvailable: true, mainAction: "查看本次结果" }] }, selectedPublicRunToken: "run:1", onSelect: () => {}, onClose: () => {} }));
...
const identityHtml = render(element("R7PublicResultIdentityStrip", { identity: { mode_text: "日常监查", data_cutoff_text: "2026-08-28", site_scope_text: "中心 006、中心 010" } }));
...
check((identityHtml.match(/data-r7-result-identity/g) || []).length === 1, "result identity strip is a single non-overlapping band");
...
const centerIdentityHtml = render(element("R7PublicResultIdentityStrip", { identity: { mode_text: "日常监查", data_cutoff_text: "2026-08-28", site_scope_text: "中心 006、中心 010" }, siteScopeText: "中心 006" }));
...
const publicProgressHtml = render(element("R7PublicProgressSurface", {
  progress: {
    runState: "completed",
    runStatusText: "分析已结束",
...
const css = fs.readFileSync(path.join(here, "medicalMonitoringR7ProductLoop.css"), "utf8");
...
console.log(`medicalMonitoringR7ProductLoopRender: ${passed} passed`);
