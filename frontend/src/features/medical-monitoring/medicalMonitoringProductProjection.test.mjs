...
import {
  MONITORING_PRODUCT_HISTORY_FIELDS,
  MONITORING_PUBLIC_RESULT_UNAVAILABLE_TEXT,
  computeMonitoringPublicResponseDigest,
  normalizeMonitoringHistory,
  normalizeMonitoringPublicProgress,
  normalizeMonitoringResultEntry,
  ...
  safeValidateMonitoringPublicResultEnvelope,
  validateMonitoringPublicResultEnvelope,
...
check(
  projectMonitoringSetupOptions({ ...setupPayload, modes: [...setupPayload.modes, mode("daily-unknown")] }, { projectId: "project-a" }).kind === "invalid",
  "a forged fourth mode fails closed instead of becoming a card",
);
check(
  projectMonitoringSetupOptions({ ...setupPayload, project_id: "other" }, { projectId: "project-a" }).kind === "invalid",
  "cross-project setup responses fail closed",
);
check(
  projectMonitoringSetupOptions({ ...setupPayload, current_data: { ...setupPayload.current_data, snapshot_ref: "internal" } }, { projectId: "project-a" }).kind === "invalid",
...
const history = projectMonitoringHistory({ runs: [historyRow] }, { projectId: "project-a" });
...
check(
  normalizeMonitoringHistory({ runs: [{ ...historyRow, percent: 100 }] }, { projectId: "project-a" }).ok === false,
  "history rejects percentage or other extra fields",
);
check(
  projectMonitoringHistory({ runs: [{ ...historyRow, run_id: "internal" }] }, { projectId: "project-a" }).kind === "invalid",
  "history rejects internal run identity",
);
check(MONITORING_PRODUCT_HISTORY_FIELDS.length === 8, "history field registry is exactly eight fields");
const resultEntry = projectMonitoringResultEntry({
...
const progress = projectMonitoringPublicProgress(progressPayload, { publicRunToken: "run:001" });
...
const availableProgress = projectMonitoringPublicProgress({ ...progressPayload, publication_state: "available", result_available: true }, { publicRunToken: "run:001" });
check(availableProgress.poll.active === false, "available publication stops polling");
check(
  projectMonitoringPublicProgress({ ...progressPayload, run_id: "internal" }, { publicRunToken: "run:001" }).kind === "invalid",
  "public progress rejects internal run identity",
);
check(
  normalizeMonitoringPublicProgress(progressPayload, {}).ok === false,
...
const digest = await computeMonitoringPublicResponseDigest(baseEnvelope);
const envelope = { ...baseEnvelope, response_digest: digest };
const valid = validateMonitoringPublicResultEnvelope(envelope, {
  projectId: "project-a",
...
check(projectMonitoringResultContext(envelope, { projectId: "project-a" }).kind === "result", "result envelope projects to a result context");
check(
  safeValidateMonitoringPublicResultEnvelope({ ...envelope, identity: { ...envelope.identity, run_ref: "internal" } }, { projectId: "project-a" }).ok === false,
  "public envelope rejects internal R5/run fields",
);
check(
  safeValidateMonitoringPublicResultEnvelope({ ...envelope, response_digest: "bad" }, { projectId: "project-a" }).ok === false,
  "public envelope rejects malformed response digests",
);
check(
  safeValidateMonitoringPublicResultEnvelope({ ...envelope, extra: true }, { projectId: "project-a" }).ok === false,
  "public envelope rejects unknown top-level keys",
);
check(
  safeValidateMonitoringPublicResultEnvelope({ ...envelope, identity: { ...envelope.identity, project_ref: "other" } }, { projectId: "project-a" }).ok === false,
  "public envelope rejects cross-project identity",
);
await verifyMonitoringPublicResultEnvelope(envelope, { projectId: "project-a", resultContextToken: "result-context:abc" });
passed += 1;
await assert.rejects(
  verifyMonitoringPublicResultEnvelope({ ...envelope, projection: { changed: true } }, { projectId: "project-a" }),
  (error) => error.code === "public_response_digest_mismatch",
);
passed += 1;
check(
  projectMonitoringResultContext({ ...envelope, response_digest: "bad" }, { projectId: "project-a" }).text === MONITORING_PUBLIC_RESULT_UNAVAILABLE_TEXT,
