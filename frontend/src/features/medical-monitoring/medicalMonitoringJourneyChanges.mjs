// Pure continuity-to-Journey same-identity functions:
...
export const MONITORING_JOURNEY_DRAWER_KEYS = Object.freeze([
  "event_ref",
  "risk_instance_ref",
  "risk_anchor_ref",
  "visit_ref",
...
export const MONITORING_JOURNEY_CHANGE_TONES = Object.freeze([
  "danger",
  "warning",
  "success",
  "info",
...
export const MONITORING_JOURNEY_DRAWER_SECTION_ORDER = Object.freeze([
  "title",
  "event_category",
  "risk_level",
  "change",
...
export const MONITORING_JOURNEY_DRAWER_FALLBACK_TEXTS = Object.freeze({
  eventCategory: "类别待确认",
  riskLevel: "风险等级待确认",
  change: "本轮变化待确认",
  date: "日期待确认",
...
export const MONITORING_JOURNEY_TRUNCATION_PREFIX = "本轮变化较多，当前仅显示服务端已返回的前";
export const MONITORING_JOURNEY_TRUNCATION_SUFFIX = "条；请返回项目或中心概览查看完整范围说明。";

const MONITORING_JOURNEY_CHANGE_ICONS = Object.freeze({
  new: "CirclePlus",
...
const MONITORING_JOURNEY_CHANGE_TONES_BY_KIND = Object.freeze({
  new: "info",
  upgraded: "danger",
  continued: "neutral",
  downgraded: "success",
...
// seven-kind set.
export const MONITORING_JOURNEY_CHANGE_KIND_VISUAL = Object.freeze(
  Object.fromEntries(
    MONITORING_CONTINUITY_CHANGE_KINDS.map((kind) => [
      kind,
      Object.freeze({
        icon: MONITORING_JOURNEY_CHANGE_ICONS[kind],
        label: MONITORING_CONTINUITY_CHANGE_KIND_TEXTS[kind],
        tone: MONITORING_JOURNEY_CHANGE_TONES_BY_KIND[kind],
      }),
    ]),
  ),
);
...
export { monitoringContinuityRowSeverityLabel as monitoringJourneySeverityLabel };
...
export function monitoringJourneyAxisWindow(resultPayload, route = {}) {
...
export function monitoringJourneyWindowContains(row, window) {
...
export function filterMonitoringContinuityRowsForJourney(
  rows,
  ...
  return freeze((Array.isArray(rows) ? rows : []).filter((row) => {
    ...
    return monitoringJourneyWindowContains(row, window);
  }));
}
...
export function monitoringJourneyContinuityRows(envelope, resultPayload, route = {}) {
  const value = envelope?.ok ? envelope.value : null;
  if (!value || value.kind !== "continuity") return [];
  const axis = monitoringJourneyAxisWindow(resultPayload, route);
  if (!axis) return [];
  return filterMonitoringContinuityRowsForJourney(value.comparison.rows, {
...
export function bindMonitoringContinuityRowsToJourney(rows, events = [], risks = []) {
...
export function monitoringEventChangeRows(binding, eventRef) {
...
export function monitoringRiskChangeRows(binding, riskInstanceRef) {
...
export function monitoringEventChangeMarker(rows) {
  ...
  const visual = monitoringJourneyChangeKindVisual(primary.change_kind);
...
export function monitoringJourneyChangeKindVisual(changeKind) {
  const entry = MONITORING_JOURNEY_CHANGE_KIND_VISUAL[clean(changeKind)];
  return entry || null;
}
...
export function monitoringJourneyDrawerCurrentRow(rows, route = {}) {
  const source = Array.isArray(rows) ? rows.filter((row) => row && typeof row === "object") : [];
  if (!source.length) return null;
...
export function monitoringJourneyDrawerClosePatch(route = {}) {
  if (!route || typeof route !== "object") return Object.freeze({});
  const next = { ...route };
  for (const key of MONITORING_JOURNEY_DRAWER_KEYS) next[key] = "";
  return freeze(next);
}
...
export function monitoringJourneyTruncationText(comparison) {
  ...
  return `${MONITORING_JOURNEY_TRUNCATION_PREFIX} ${shown} ${MONITORING_JOURNEY_TRUNCATION_SUFFIX}`;
}
...
export function monitoringJourneyBeforeAfterModel(comparison, row) {
  if (comparison && comparison.comparison_text === MONITORING_CONTINUITY_FIRST_ANALYSIS_TEXT) {
    return Object.freeze({ state: "first_analysis", text: MONITORING_CONTINUITY_FIRST_ANALYSIS_TEXT });
  }
  if (!row || typeof row !== "object" || row.object_type !== "risk") {
    return Object.freeze({ state: "none", text: MONITORING_JOURNEY_DRAWER_FALLBACK_TEXTS.beforeAfter });
  }
  const parts = [];
...
// currently selected risk's evidenceSummary.query_draft; missing or empty
// values yield "" so the drawer shows the fixed fallback text. Continuity
// query_draft object rows are never cross-guessed.
export function monitoringJourneyQueryDraft(risk) {
