...
// §7.4): the continuity read lives in MedicalMonitoringR7ProductLoop and is a
...
const source = fs.readFileSync(path.join(here, "MedicalMonitoringR7ProductLoop.jsx"), "utf8");
...
// --- imports: the continuity surface is wired into the loop ---
{
  ...
  check(
    src.includes(needle(`import { MedicalMonitoringR7ContinuityPanel } from "./MedicalMonitoringR7ContinuityPanel.jsx";`)),
...
{
  check(src.includes("MedicalMonitoringR7ContinuityPanel"), "panel is rendered by the loop");
  check(
  ...
  check(
    (src.match(/MedicalMonitoringR7ContinuityPanel/g) || []).length >= 3,
    "continuity panel remains wired (import + tamper-lead + comparable-follow)",
  );
  check(
    src.lastIndexOf("MedicalMonitoringR7ContinuityPanel") > src.indexOf(needle(`{currentResultView}`)),
...
// --- 08C-3 subject-view wiring: verified continuity props reach the journey ---
{
  ...
  check(
    src.includes(needle(`import { r7JourneyDrawerClosePatch } from "./medicalMonitoringR7JourneyChanges.mjs";`)),
...
{
  const { r7ContinuityRowJourneyTarget, r7ContinuityRowSourceTarget } = await import("./medicalMonitoringR7ContinuityFilter.mjs");
  const riskRow = {
    object_type: "risk",
    subject_ref: "S/01",
    site_ref: "site/01",
...
console.log(`medicalMonitoringR7ContinuityIntegration: ${passed} passed`);
