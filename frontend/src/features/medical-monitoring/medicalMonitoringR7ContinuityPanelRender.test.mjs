...
const bundlePath = path.join(here, ".r7-continuity-panel-render-bundle.cjs");
buildSync({
  entryPoints: [path.join(here, "medicalMonitoringR7ContinuityPanelRender.test.jsx")],
  bundle: true,
  outfile: bundlePath,
  format: "cjs",
  platform: "node",
...
check(renders.loading.includes('data-r7-continuity-state="loading"'), "loading state hook present");
...
check(!renders.loading.includes("r7-continuity-counts"), "loading renders no counts");
check(!renders.loading.includes("r7-continuity-list"), "loading renders no rows");
check(!renders.loading.includes("本轮变化暂不可查看"), "loading is not the unavailable state");
passed += 6;

// --- unavailable: single degrade text (contract §5) ---
check(renders.unavailableDefault.includes('data-r7-continuity-state="unavailable"'), "unavailable state hook present");
...
check(renders.unavailableWins.includes("自定义错误提示") && !renders.unavailableWins.includes("r7-continuity-counts"), "unavailable takes precedence over a stale comparison");
check(!renders.unavailableDefault.includes("r7-continuity-list"), "unavailable renders no rows");
passed += 6;

// --- ready: title, subtitle, five key counts, closed filter chips ---
check(renders.ready.includes('data-r7-continuity-state="ready"'), "ready state hook present");
...
check(renders.ready.includes("r7-continuity-count is-key"), "中高风险 is the single semantic emphasis");
...
check(renders.ready.includes("<span class=\"r7-continuity-tag is-object\">风险</span>"), "object tag uses the public Chinese label");
...
{
  const css = fs.readFileSync(path.join(here, "medicalMonitoringR7ContinuityPanel.css"), "utf8");
  check(css.includes("var(--r7-product-line)") && css.includes("var(--r7-product-orange)"), "panel reuses the r7 product design tokens");
  check(css.includes("font-variant-numeric: tabular-nums"), "key counts use tabular numerals");
  check(css.includes(".r7-continuity-count.is-key") && css.includes("var(--r7-product-orange)"), "中高风险 is the orange semantic emphasis");
...
console.log(`medicalMonitoringR7ContinuityPanelRender: ${passed} passed`);
