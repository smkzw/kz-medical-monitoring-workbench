...
assert.equal(pageSource.includes('data-r5-zoom={zoomLevel}'), true, "page exposes the semantic zoom state to the presentation layer");
...
assert.equal(pageSource.includes('data-r5-evidence-field="record_ref"'), true, "source evidence visibly exposes the exact record reference");
assert.equal(pageSource.includes('data-r5-evidence-field="canonical_location"'), true, "source evidence visibly exposes the exact canonical location");
assert.equal(pageSource.includes('data-r5-evidence-field="excerpt"'), true, "source evidence visibly exposes the exact citation excerpt");
...
assert.equal(pageSource.includes("r5-phrase-keep"), true, "density journey keeps 按缩放级别聚合 as an unbreakable phrase");
...
assert.equal(styleSource.includes('.r5-page[data-r5-zoom="-1"]'), true, "compact semantic zoom changes track detail density");
assert.equal(styleSource.includes('.r5-page[data-r5-zoom="1"] .r5-track-event-source'), true, "detailed semantic zoom reveals source locator detail");
assert.equal(styleSource.includes(".r5-timeline-canvas"), true, "styles define the shared timeline canvas");
assert.equal(styleSource.includes(".r5-pending-date-zone"), true, "styles isolate the pending-date surface");
console.log("medicalMonitoringR5ProductContract: production source checks passed");
