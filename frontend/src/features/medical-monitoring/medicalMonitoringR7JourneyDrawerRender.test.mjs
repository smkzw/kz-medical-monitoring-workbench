...
buildSync({
  entryPoints: [path.join(here, "medicalMonitoringR7JourneyDrawerRender.test.jsx")],
  bundle: true,
  outfile: bundlePath,
  format: "cjs",
  platform: "node",
...
{
  const css = fs.readFileSync(path.join(here, "medicalMonitoringR7JourneyDrawer.css"), "utf8");
...
{
  const pageSource = fs.readFileSync(path.join(here, "MedicalMonitoringR5Page.jsx"), "utf8");
...
console.log(`medicalMonitoringR7JourneyDrawerRender: ${passed} passed`);
