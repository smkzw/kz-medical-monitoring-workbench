// Pure projection for the live monitoring progress surface. The server
...
export const MONITORING_RUN_STATES = Object.freeze([
  "waiting_start",
  "running",
  "stopping",
...
export const MONITORING_POLLING_RUN_STATES = Object.freeze(["running", "stopping"]);
export const MONITORING_POLL_INTERVAL_MS = 2000;
export const MONITORING_REFRESH_BACKOFF_MS = Object.freeze([2000, 5000, 10000, 30000]);

export const MONITORING_CURRENT_WORK_LIMIT = 3;
export const MONITORING_LATEST_UPDATES_LIMIT = 5;

export const MONITORING_NO_RUN_TEXT = "尚无本次监查";
export const MONITORING_NOT_PREPARED_TEXT = "本次监查范围尚未准备";
export const MONITORING_FORBIDDEN_ACCOUNT_TEXT = "当前账号不能操作本次监查运行";
export const MONITORING_REFRESH_FAILED_TEXT = "进度刷新失败";
export const MONITORING_PREPARING_NEXT_WORK_TEXT = "正在准备下一项分析";

export const MONITORING_OUTCOME_LABELS = Object.freeze({
  stopping: "正在停止",
...
export const MONITORING_ACTION_ROUTES = Object.freeze({
  开始: "start",
  停止: "cancel",
  继续: "resume",
});
...
export const MONITORING_FORBIDDEN_USER_VISIBLE_TERMS = Object.freeze([
  "正式事实",
  "候选信号",
  "只读",
  "provider",
...
export function isMonitoringRunState(value) {
  return MONITORING_RUN_STATE_SET.has(value);
}

export function monitoringPollDecision(runState) {
  const active = MONITORING_POLLING_RUN_STATES.includes(runState);
  return Object.freeze({
    active,
    intervalMs: active ? MONITORING_POLL_INTERVAL_MS : 0,
  });
}

export function monitoringRefreshBackoffMs(consecutiveFailures) {
  const failures = Number.isInteger(consecutiveFailures) && consecutiveFailures > 0
    ? consecutiveFailures
    : 1;
  return MONITORING_REFRESH_BACKOFF_MS[Math.min(failures, MONITORING_REFRESH_BACKOFF_MS.length) - 1];
}

// Extract the request context from the canonical route state. Returns null
...
export function monitoringProgressRequestContext(canonical) {
...
export function projectMonitoringProgressActions(availableActions) {
  const actions = [];
  for (const label of cleanList(availableActions)) {
    const text = cleanText(label);
    const route = MONITORING_ACTION_ROUTES[text];
    if (route) actions.push(Object.freeze({ label: text, action: route }));
  }
  return Object.freeze(actions);
}

export function projectMonitoringProgress(payload) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return Object.freeze({ kind: "invalid", text: MONITORING_REFRESH_FAILED_TEXT });
  }
  const runState = cleanText(payload.run_state);
  if (!isMonitoringRunState(runState)) {
    return Object.freeze({ kind: "invalid", text: MONITORING_REFRESH_FAILED_TEXT });
  }
  ...
  const currentWork = Object.freeze(currentWorkAll.slice(0, MONITORING_CURRENT_WORK_LIMIT));
  const currentWorkRemaining = Math.max(0, currentWorkAll.length - MONITORING_CURRENT_WORK_LIMIT);

  const selectedUpdates = cleanList(payload.latest_updates)
    .slice(0, MONITORING_LATEST_UPDATES_LIMIT);
  ...
  return Object.freeze({
    ...
    outcomeLabel: MONITORING_OUTCOME_LABELS[runState] || "",
    completed,
    total,
    percent,
    stageProgress,
    ...
    actions: projectMonitoringProgressActions(payload.available_actions),
    poll: monitoringPollDecision(runState),
  });
}
...
export function projectMonitoringProgressError(error) {
  const code = errorCode(error);
  if (code === "run_binding_not_found") {
    return Object.freeze({ kind: "empty", emptyReason: "no_run", text: MONITORING_NO_RUN_TEXT });
  }
  if (code === "execution_not_prepared") {
    return Object.freeze({
      kind: "empty",
      emptyReason: "not_prepared",
      text: MONITORING_NOT_PREPARED_TEXT,
    });
  }
  ...
  if (status === 403) {
    return Object.freeze({
      kind: "forbidden",
      text: MONITORING_FORBIDDEN_ACCOUNT_TEXT,
      serverText,
    });
  }
  return Object.freeze({
    kind: "error",
    text: serverText || MONITORING_REFRESH_FAILED_TEXT,
  });
}
...
// the current view.
export function createMonitoringProgressGate() {
  let epoch = 0;
  let identity = "";
  return Object.freeze({
...
export function collectMonitoringUserVisibleTexts(view) {
...
export function findMonitoringForbiddenTerms(texts) {
  const hits = [];
  for (const text of cleanList(texts)) {
    const haystack = String(text).toLowerCase();
    for (const term of MONITORING_FORBIDDEN_USER_VISIBLE_TERMS) {
