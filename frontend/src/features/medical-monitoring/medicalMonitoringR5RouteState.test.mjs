...
import {
  ...
  MEDICAL_MONITORING_R5_EPHEMERAL_KEYS,
  ...
  splitMedicalMonitoringR5State,
  validateMedicalMonitoringR5Target,
...
const validSubject = parseMedicalMonitoringR5RouteState(
...
check(validSubject.isR5 && validSubject.valid, "accepts a complete subject deep link");
...
const serialized = serializeMedicalMonitoringR5RouteState(validSubject.canonical);
...
const unknownQuery = parseMedicalMonitoringR5RouteState(
  "?project_id=synthetic-project-r5-s7&view=overview&debug=1",
);
check(unknownQuery.isR5 && !unknownQuery.valid, "rejects an unknown R5 query key");
check(unknownQuery.errorCode === "UNKNOWN_QUERY_KEY", "returns a distinguished unknown-key failure");

const partialOverview = parseMedicalMonitoringR5RouteState(
  "?project_id=synthetic-project-r5-s7&view=overview&run_id=run-only",
);
check(!partialOverview.valid && partialOverview.errorCode === "R5_IDENTITY_INCOMPLETE", "rejects a partial overview authority identity");

const missingIdentity = parseMedicalMonitoringR5RouteState(
  "?project_id=synthetic-project-r5-s7&view=journey",
);
check(missingIdentity.isR5 && !missingIdentity.valid, "does not enter a subject view without target identity");
check(missingIdentity.errorCode === "R5_IDENTITY_INCOMPLETE", "returns a distinguished incomplete-identity failure");

const subjectWithoutReturnContext = parseMedicalMonitoringR5RouteState(
  "?project_id=synthetic-project-r5-s7&run_id=synthetic-run-r5-20260820"
    + "&snapshot_id=synthetic-snapshot-r5-20260820&cutoff=2026-08-20"
    + "&scope=subject&site_id=synthetic-site-01&subject_id=synthetic-subject-001"
    + "&view=journey&spine_id=synthetic-spine-001&axis_mode=calendar"
...
const legacy = parseMedicalMonitoringR5RouteState("?project_id=p1&view=checklist");
check(!legacy.isR5 && legacy.status === "legacy", "leaves the legacy view outside R5 route handling");

const switched = routeStateForMedicalMonitoringR5View(validSubject.canonical, "profile");
...
const evidence = routeStateForMedicalMonitoringR5View(switched, "evidence", {
...
const returnedOverview = routeStateForMedicalMonitoringR5View(validSubject.canonical, "overview");
...
const split = splitMedicalMonitoringR5State({ ...validSubject.canonical, inspector_width: 360, focus_ref: "risk-1", scroll_refs: ["risk-list"] });
check(MEDICAL_MONITORING_R5_CANONICAL_KEYS.includes("project_ref"), "declares the frozen project identity key");
check(MEDICAL_MONITORING_R5_EPHEMERAL_KEYS.includes("inspector_width"), "declares inspector width as ephemeral state");
...
const targetCheck = validateMedicalMonitoringR5Target(validSubject.canonical);
check(targetCheck.valid, "validates an exact target without nearest substitution");
const invalidTarget = validateMedicalMonitoringR5Target({ ...validSubject.canonical, risk_instance_ref: "" });
check(!invalidTarget.valid, "rejects a risk target with an incomplete identity pair");

check(MEDICAL_MONITORING_R5_CANONICAL_KEYS.includes("flow_stage_ref")
  && MEDICAL_MONITORING_R5_CANONICAL_KEYS.includes("flow_node_metric")
  && MEDICAL_MONITORING_R5_CANONICAL_KEYS.includes("flow_link_ref")
  && MEDICAL_MONITORING_R5_CANONICAL_KEYS.includes("flow_risk_band"), "declares the frozen flow route keys as canonical identity");

const flowStageQuery = parseMedicalMonitoringR5RouteState(
  "?project_id=synthetic-project-r5-s7&view=overview&flow_stage_ref=flow-stage-treatment&flow_node_metric=reached",
);
check(flowStageQuery.isR5 && flowStageQuery.valid, "accepts a stage flow filter on the project overview");
...
const flowStageDefaultMetric = parseMedicalMonitoringR5RouteState(
...
const flowLinkQuery = parseMedicalMonitoringR5RouteState(
...
const stageAfterLink = parseMedicalMonitoringR5RouteState(
...
const linkAfterStage = parseMedicalMonitoringR5RouteState(
...
const flowBandQuery = parseMedicalMonitoringR5RouteState(
...
const badMetric = parseMedicalMonitoringR5RouteState(
  "?project_id=synthetic-project-r5-s7&view=overview&flow_stage_ref=flow-stage-treatment&flow_node_metric=total",
);
check(!badMetric.valid && badMetric.errorCode === "R5_IDENTITY_INCOMPLETE" && badMetric.errorFields.includes("flow_node_metric"), "rejects a flow metric outside the closed registry");
const badBand = parseMedicalMonitoringR5RouteState(
  "?project_id=synthetic-project-r5-s7&view=overview&flow_stage_ref=flow-stage-treatment&flow_risk_band=low",
);
check(!badBand.valid && badBand.errorCode === "R5_IDENTITY_INCOMPLETE" && badBand.errorFields.includes("flow_risk_band"), "rejects a flow risk band outside the closed registry");

const flowSerialized = serializeMedicalMonitoringR5RouteState(flowBandQuery.canonical);
...
const linkSerialized = serializeMedicalMonitoringR5RouteState(flowLinkQuery.canonical);
...
const journeyWithFlow = routeStateForMedicalMonitoringR5View(flowBandQuery.canonical, "journey", {
...
const returnedWithFlow = routeStateForMedicalMonitoringR5View(journeyWithFlow, "overview");
...
const returnedFresh = routeStateForMedicalMonitoringR5View(subjectWithoutReturnContext.canonical, "overview");
check(!returnedFresh.flow_stage_ref && !returnedFresh.flow_link_ref, "carries no flow keys when none were set");

const patchStageOverLink = routeStateForMedicalMonitoringR5View(flowLinkQuery.canonical, "overview", { flow_stage_ref: "flow-stage-new" });
...
const patchLinkOverStage = routeStateForMedicalMonitoringR5View(flowStageDefaultMetric.canonical, "overview", { flow_link_ref: "flow-link-new" });
...
const clearedFlow = routeStateForMedicalMonitoringR5View(flowBandQuery.canonical, "overview", { flow_stage_ref: "", flow_node_metric: "", flow_risk_band: "" });
...
const normalizedConflict = normalizeMedicalMonitoringR5RouteState({ flow_stage_ref: "flow-stage-a", flow_link_ref: "flow-link-b" });
...
const normalizedOrphanMetric = normalizeMedicalMonitoringR5RouteState({ flow_node_metric: "reached" });
check(!normalizedOrphanMetric.flow_node_metric, "drops an orphan flow metric without a main selection");
const normalizedBadMetric = normalizeMedicalMonitoringR5RouteState({ flow_stage_ref: "flow-stage-a", flow_node_metric: "total" });
check(normalizedBadMetric.flow_node_metric === "current", "coerces an invalid flow metric back to the default");
const normalizedEmptyBand = normalizeMedicalMonitoringR5RouteState({ flow_stage_ref: "flow-stage-a", flow_risk_band: "" });
...
const flowSplit = splitMedicalMonitoringR5State({ ...flowBandQuery.canonical, focus_ref: "flow-stage-treatment" });
...
const flowTarget = validateMedicalMonitoringR5Target(flowBandQuery.canonical);
check(flowTarget.valid, "keeps a flow-filtered overview a valid target");

const publicResult = parseMedicalMonitoringR5RouteState(
  "?project_id=project-a&result_context_token=result-context%3A1&view=overview",
);
check(publicResult.isR5 && publicResult.valid, "accepts a public result overview without internal R5 identity");
const publicSubject = parseMedicalMonitoringR5RouteState(
...
const publicSerialized = new URLSearchParams(serializeMedicalMonitoringR5RouteState(publicSubject.canonical).slice(1));
...
const publicExact = parseMedicalMonitoringR5RouteState(
  "?project_id=project-a&result_context_token=result-context%3A1&view=journey"
...
const mixedPublic = parseMedicalMonitoringR5RouteState(
  "?project_id=project-a&result_context_token=result-context%3A1&view=overview&run_id=internal",
);
const publicRiskKey = parseMedicalMonitoringR5RouteState(
...
const mixedPublicProgress = parseMedicalMonitoringR5RouteState(
...
const publicProgress = parseMedicalMonitoringR5RouteState(
  "?project_id=project-a&public_run_token=run%3A1&view=overview",
);
check(publicProgress.valid, "accepts a public progress route");
console.log(`medicalMonitoringR5RouteState: ${passed} checks passed`);
