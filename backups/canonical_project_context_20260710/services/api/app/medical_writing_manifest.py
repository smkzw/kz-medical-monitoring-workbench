...
class MedicalWritingManifestService:
    ...
    def build_manifest(self, project_id: str) -> MedicalWritingManifestResult:
        ...
        return MedicalWritingManifestResult(
            ...
            parser_notes=[
                ...
                "AI修订建议只能作为待医学批准的正式内容候选；独立AI provider未配置前不生成生产写作正文。",
            ],
        )
    ...
    def _build_package(self, config: WritingPackageConfig) -> WritingSourcePackageSummary:
        ...
        return WritingSourcePackageSummary(
            ...
            pending_medical_approval_count=sum(1 for section in sections if section.medical_approval_status != "医学已批准"),
...
def _build_quality_gates(
    ...
    gate_items = [
        ...
        WritingQualityGate(
            ...
            status="warning",
            owner="医学经理",
            detail="当前为来源定位起点，覆盖率不代表医学批准；关键断言需绑定source_id、locator、quote或表格行列定位。",
            source_refs=[],
        ),
        WritingQualityGate(
            ...
            detail=(
                ...
                "独立AI provider未配置；方案解构、证据覆盖和医学写作修订不会自动调用AI。"
            ),
            source_refs=["codex_runtime_dependency=false"],
        ),
        WritingQualityGate(
            ...
            status="blocked",
            owner="医学经理+医学总监",
            detail="未关闭AI修订、证据覆盖不足、高风险表述未清理或审批状态不是医学已批准时，不可生成正式导出包；当前最多支持审阅版DOCX准备检查。",
            source_refs=[],
        ),
        WritingQualityGate(
            ...
            status="ok",
            owner="医学写作",
            detail="manifest仅显示来源、候选、质量门和审批边界，不输出自动完成、直接递交或最终结论类表述。",
            source_refs=[],
        ),
    ]
    return gate_items
...
def _extract_protocol_metadata(document: ProtocolTextDocument, config: WritingPackageConfig) -> Dict[str, str]:
    ...
    version = _first_label_value(joined, ("方案版本号",)) or ""
    date = _first_label_value(joined, ("方案版本日期",)) or ""
    return {
