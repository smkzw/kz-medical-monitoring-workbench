...
class SafetyPvManifestService:
    ...
    def build_manifest(self, project_id: str) -> SafetyPvManifestResult:
        generated_at = datetime.now(timezone.utc)
        package_summaries = [self._build_package(config) for config in self.packages]
    ...
    def _build_package(self, config: SafetyPackageConfig) -> SafetySourcePackageSummary:
        warnings: List[str] = []
        listing_domains, listing_metrics = _scan_listing_workbook(config, warnings)
...
def _scan_listing_workbook(
    ...
    for sheet_name in workbook.sheetnames:
        ...
        subject_idx = _column_index(headers, "USUBJID")
        ...
        metrics["subject_ids"].update(subjects)
        metrics["site_ids"].update(sites)
        _update_listing_metrics(sheet_name, sheet, headers, metrics)
...
def _update_listing_metrics(sheet_name: str, sheet, headers: List[str], metrics: Dict[str, object]) -> None:
    ae_term_idx = _column_index(headers, "AETERM")
    ae_ser_idx = _column_index(headers, "AESER")
    ae_rel_idx = _column_index(headers, "AEREL")
    lb_sig_idx = _column_index(headers, "LBSIG")
    eg_sig_idx = _column_index(headers, "EGSIG")
    eg_ae_idx = _column_index(headers, "EGAEMH1")
    cm_term_idx = _column_index(headers, "CMTRT")
    for row in sheet.iter_rows(min_row=2, values_only=True):
        if sheet_name == "AE" and ae_term_idx is not None and _cell_text(row[ae_term_idx]):
        ...
        if lb_sig_idx is not None and _is_clinically_significant(_cell_text(row[lb_sig_idx])):
            metrics["lab_cs_count"] = int(metrics["lab_cs_count"]) + 1
            metrics["lab_cs_by_sheet"][sheet_name] += 1
        if eg_sig_idx is not None and _is_clinically_significant(_cell_text(row[eg_sig_idx])):
        ...
        if cm_term_idx is not None and _cell_text(row[cm_term_idx]):
    ...
    for file_path in source_files:
        ...
        documents.append(
            SafetySourceDocumentSummary(
                ...
                source_package=config.package_id,
                relative_path=_public_relative(file_path),
    ...
    if lab_cs_count:
        ...
        candidates.append(
            SafetySignalCandidate(
                ...
                evidence_locators=[f"{top_labs} / LBSIG"],
    ...
    if eg_cs_count:
        candidates.append(
            SafetySignalCandidate(
                ...
                source_package=config.package_id,
                source_domains=["EG"],
                evidence_locators=["EG sheet / EGSIG, EGINDC, EGAEMH, EGAEMH1"],
    ...
    if medication_count:
        candidates.append(
            SafetySignalCandidate(
                ...
                evidence_locators=["CM/CM1/JWYY/HBYY sheets / CMTRT, CMINDC1, CMAEMH"],
...
def _build_quality_gates(
    ...
    has_ae = any(domain.sheet_name == "AE" for domain in domains)
...
def _headers(sheet) -> List[str]:
    ...
    try:
        ...
        return []
    return [_normalize_header(value) for value in first]
...
def _domain_code(sheet_name: str, headers: List[str]) -> str:
    domain_idx = _column_index(headers, "DOMAIN")
...
def _domain_parser_notes(sheet_name: str, headers: List[str], row_count: int) -> List[str]:
    ...
    if sheet_name.upper().startswith("LB"):
        notes.append("NCS/CS判定按LBSIG精确区分，避免把异常无临床意义误计为候选。")
    if sheet_name.upper() == "AE":
...
def _public_relative(file_path: Path) -> str:
    parts = file_path.parts[-4:]
    return "/".join(parts)
