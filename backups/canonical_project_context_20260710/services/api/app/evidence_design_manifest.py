...
class EvidenceDesignManifestService:
    ...
    def build_manifest(self, project_id: str) -> EvidenceDesignManifestResult:
        ...
        return EvidenceDesignManifestResult(
            ...
            total_products=sum(len(package.products) for package in package_summaries),
            total_trials=sum(len(package.trial_designs) for package in package_summaries),
            total_documents=sum(len(package.documents) for package in package_summaries),
            total_result_rows=sum(len(package.efficacy_results) + len(package.safety_results) for package in package_summaries),
            ...
            parser_notes=[
                "P0从CRSwNP主数据库CSV和原始/近原始资料索引重建证据工作面，不读取既有深度报告正文作为生产输入。",
                "Protocol、SAP、Publication和监管资料当前以文件级登记与结构化索引为主；正式章节级抽取需独立OCR/LLM服务后续接入。",
                "PICOS输出是待医学确认的设计问题队列，可流转到医学写作，但不代表方案已完成医学批准。",
            ],
        )

    def _build_package(self, config: EvidenceDesignPackageConfig) -> EvidenceDesignPackageSummary:
        warnings: List[str] = []
