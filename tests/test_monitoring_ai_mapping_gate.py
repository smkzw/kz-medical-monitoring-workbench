...
def test_gate_contract_aligns_registry_preset_and_profile() -> None:
    ...
    assert contract.provider == MONITORING_C3_MAPPING_PROVIDER == preset["provider"]
    assert contract.model == MONITORING_C3_MAPPING_MODEL == preset["default_model"]
    assert contract.base_url == ZHIPU_CODING_PLAN_BASE_URL == profile.base_url
...
def test_monitoring_role_defaults_to_zhipu_without_changing_independent_ai(tmp_path) -> None:
