...
# Keep the accepted POC import seam local to R7.  These paths are read-only
# sources; the bridge never mutates either adjacent package.
_POC_ROOT = Path(__file__).resolve().parents[3]
_R1_SRC = _POC_ROOT / "medical_monitoring_ai_native_r1" / "src"
_R6_SRC = _POC_ROOT / "medical_monitoring_ai_native_r6" / "src"
for _source in (_R1_SRC, _R6_SRC):
    if _source.is_dir() and str(_source) not in sys.path:
        sys.path.insert(0, str(_source))

from mm_r1.adapters import ImmutableAdapterBinding  # noqa: E402
from mm_r1.capability_runtime import (  # noqa: E402
...
from mm_r1.domain import CoverageUnit  # noqa: E402
