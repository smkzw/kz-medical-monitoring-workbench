...
# Ensure adjacent POC source roots (mm_r1, mm_r5, mm_r6, mm_r4, mm_r2) are in sys.path
_CUR_PATH = Path(__file__).resolve()
for _p in _CUR_PATH.parents:
    if (_p / "poc").is_dir():
        for _r_name in ("r7", "r6", "r5", "r4", "r3", "r3_rule_ai", "r2", "r1"):
            _r_dir = _p / "poc" / f"medical_monitoring_ai_native_{_r_name}" / "src"
            if _r_dir.is_dir() and str(_r_dir) not in sys.path:
                sys.path.insert(0, str(_r_dir))
        break
