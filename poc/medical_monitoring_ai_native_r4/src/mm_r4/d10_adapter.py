"""R4-D10 test-only frozen-artifact adapter and oracle leaf vocabulary.

This module is the ONLY component allowed to read the frozen D10 artifact
chain (catalog / oracle / registry / quota).  It:

* pins the frozen file SHA-256 digests so a stale artifact fails closed before
  any case is parsed;
* converts the frozen typed catalog ``typed_input`` JSON into the immutable
  ``d10_contracts`` typed objects (``build_typed_input``);
* maps a deterministic ``d10_evaluator`` result back onto the independent
  oracle's leaf vocabulary (``medical_unit`` / gate / trace / source leaves)
  so the test suite can demand exact per-key parity.

The production runtime modules (``d10_contracts.py`` / ``d10_evaluator.py``)
never import this module and never read the frozen artifacts.  This module is
imported by the test suite only.

Clean-semantics boundary (worker_01): the conversion is purely structural --
it copies the exact-key JSON onto the closed dataclasses and never infers
identity, treatment group, endpoint, severity or same-origin relationships
from Chinese titles, free text, display order, center names or model
summaries.  ``mutation_context`` / ``anti_overfit_variant`` are carried as
opaque shape-only audit metadata and are never read by the evaluator's
decision path.
"""
...
# ---------------------------------------------------------------------------
# Frozen artifact locations and content pins (artifact freeze 2026-08-17)
# ---------------------------------------------------------------------------

# ``d10_adapter.py`` lives at ``<workspace>/poc/.../src/mm_r4/``; the frozen
# artifacts live at the workspace root ``<workspace>/reviews/``.
_WORKSPACE_ROOT = Path(__file__).resolve().parents[4]
_REVIEWS = _WORKSPACE_ROOT / "reviews"

FROZEN_ARTIFACTS: Tuple[Tuple[str, str], ...] = (
    ("medical_monitoring_r4_d10_project_signal_slice_contract_v0_6_20260816.md",
     "c613bb7cad82caa6fa477ee2dd28bfca48b805b73503c646401a0aa237deff95"),
    ("medical_monitoring_r4_d10_typed_fixture_catalog_v1_20260816.json",
     "40ce96b2e5c188167cacbe03a8b5a80260886276cbacd1edbd1261f9b7927939"),
    ("medical_monitoring_r4_d10_expected_outcome_oracle_v1_20260816.json",
     "435492cd2c86ef0e3a6b6061f9cd992d12396579a07b8f186c3c533fd9260ec3"),
    ("medical_monitoring_r4_d10_challenge_manifest_registry_v1_20260816.json",
     "2f2763c5b351331ab103882dab72d996f10ede08e45d627a4baacb3fdb3709d6"),
    ("medical_monitoring_r4_d10_partition_quota_manifest_v1_20260816.json",
     "7041cd4167ba5c604d20bfefbbe9c6aaa136ed3d0d933d064f80cf819ba88ffa"),
    ("medical_monitoring_r4_d10_fixture_authority_registry_v1_20260816.json",
     "191f4b4fcddfebfbc29d48111bf8b45477d435ae62def0199bed0edd587fa7f0"),
)

CATALOG_PATH = _REVIEWS / "medical_monitoring_r4_d10_typed_fixture_catalog_v1_20260816.json"
ORACLE_PATH = _REVIEWS / "medical_monitoring_r4_d10_expected_outcome_oracle_v1_20260816.json"
REGISTRY_PATH = _REVIEWS / "medical_monitoring_r4_d10_challenge_manifest_registry_v1_20260816.json"
QUOTA_PATH = _REVIEWS / "medical_monitoring_r4_d10_partition_quota_manifest_v1_20260816.json"
AUTHORITY_PATH = _REVIEWS / "medical_monitoring_r4_d10_fixture_authority_registry_v1_20260816.json"

CASE_COUNT = 312


# ---------------------------------------------------------------------------
# Frozen artifact loading (test-only)
# ---------------------------------------------------------------------------
...
def load_artifacts() -> Tuple[Any, Any, Any, Any]:
    """Return (catalog, oracle, registry, quota) parsed from the frozen files."""
    return (
        load_json(CATALOG_PATH),
        load_json(ORACLE_PATH),
        load_json(REGISTRY_PATH),
        load_json(QUOTA_PATH),
    )


def load_authority() -> Any:
    """Return the frozen fixture-authority registry (test-only)."""
    return load_json(AUTHORITY_PATH)


def build_authority(entry: Mapping[str, Any]) -> D10EvaluationAuthority:
    """Construct the immutable runtime authority from a fixture-authority
    registry entry.  Carries only decisive explicit authority facts -- the
    accepted envelope identity, the exact accepted source revision-content
    pairs and locator set hash, and the decisive ModelEvidence pins."""
    pairs = tuple(SourceRevisionPair(
        revision_id=str(pair["revision_id"]),
        content_hash=str(pair["content_hash"]),
    ) for pair in entry["source_revisions"])
    model = entry.get("model_evidence")
    return D10EvaluationAuthority(
        project_ref=str(entry["project_ref"]),
        run_ref=str(entry["run_ref"]),
        snapshot_ref=str(entry["snapshot_ref"]),
        accepted_source_revision_content_pairs=pairs,
        accepted_source_locator_set_hash=str(entry["source_locator_set_hash"]),
        model_binding_hash=(str(model["model_binding_hash"]) if model else None),
        model_output_hash=(str(model["output_hash"]) if model else None),
    )


def verify_frozen_hashes() -> List[str]:
    """Return the list of drifted files (empty means all pins hold)."""
    problems: List[str] = []
    for name, pin in FROZEN_ARTIFACTS:
        path = _REVIEWS / name
        actual = sha256_file(path)
        if actual != pin:
            problems.append(f"{name}: expected {pin} got {actual}")
    return problems
...
# ---------------------------------------------------------------------------
# Runtime result -> oracle leaf vocabulary (test-side mapping)
# ---------------------------------------------------------------------------

# Oracle ``medical_unit`` leaf keys in canonical sorted order.
MEDICAL_UNIT_LEAF_KEYS: Tuple[str, ...] = (
    "audience_injection_blocked", "change_cause", "change_kind", "clue_count",
    "counterevidence_rule_matches", "deep_link_target_count", "denominator_kind",
    "denominator_state", "denominator_value", "estimate_kind",
    "expected_disposition", "gate_kind", "handoff_action", "hidden_member_count",
    "hidden_site_count", "hotspot_member_refs", "leaf_kind", "lineage_relation",
    "member_expansion_state", "model_evidence_role", "numerator_affected_site_count",
    "numerator_affected_subject_count", "numerator_center_pattern_count",
    "numerator_event_or_outcome_count", "numerator_individual_risk_count",
    "numerator_member_count", "pd_wording_state", "project_signal_count",
    "query_count", "query_redundancy_decision", "rate_projection_state",
    "reason_codes", "risk_handoff_count", "signal_kind", "unit_stable_core_ref",
)

TRACE_LEAF_KEYS: Tuple[str, ...] = (
    "content_identity", "replay_byte_equal", "stable_core_ref",
    "terminal_state", "trace_kind",
)

SOURCE_LEAF_KEYS: Tuple[str, ...] = (
    "member_ref", "resolution_state", "site_stable_id",
    "source_locator_ref", "subject_stable_id",
)

FORBIDDEN_LEAF_KEYS: Tuple[str, ...] = (
    "change_kind", "expected_disposition", "gate_kind", "leaf_kind", "reason_code",
)


def result_trace_leaf(result: Any) -> Dict[str, Any]:
    """Map the first trace leaf onto the oracle ``trace`` vocabulary."""
...
def result_source_leaves(result: Any) -> List[Dict[str, Any]]:
    """Map the source leaves onto the oracle ``source`` leaf vocabulary."""
    return [{
...
def result_forbidden_leaves(result: Any) -> List[Dict[str, Any]]:
    """Map forbidden leaves onto the oracle ``forbidden`` vocabulary."""
    return [{
