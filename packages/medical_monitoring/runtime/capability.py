...
from .capability_isolation import (
    AttemptIdentityError, AttemptStage, CapabilityRuntimeError,
    HarnessIsolationBackend, HarnessIsolationPolicy, StaleAttemptError,
    TransportKind, _SANDBOX_EXEC_PATH, _path_within, _seatbelt_argv,
    ...
    def verify_effective_identity(self) -> None:
        if self.transport == TransportKind.HARNESS:
            ...
            if self.harness_isolation is not None:
                ...
                if not _SANDBOX_EXEC_PATH.is_file() or not os.access(
                    str(_SANDBOX_EXEC_PATH), os.X_OK
                ):
                    raise CapabilityRuntimeError(
                        "macos_seatbelt_r1 backend is unavailable: "
                        "/usr/bin/sandbox-exec is not executable"
                    )
...
class HarnessCapabilityRuntime(CapabilityRuntime):
    ...
    def _execute(self, request: CapabilityRequest) -> TransportExecution:
        ...
        command = (
            _seatbelt_argv(isolation, self.profile.argv, _SANDBOX_EXEC_PATH)
