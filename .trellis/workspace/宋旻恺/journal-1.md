...
### 2026-09-22 W02 决策冻结与低负担接入切片；按用户要求无损暂停

执行机制：本切片修改同一文档裁决与接入状态链，由主线程单一所有者完成；尚未关闭W02，也未触发需独立结论复核的最终验收。产品改动提交`4857ce3`。

完成内容：用户文件角色选择升级为项目/批次/清单哈希绑定的不可变版本决策，记录真实授权主体；相同选择幂等，不同选择要求版本匹配，并用批次级文件锁序列化并发写入。历史v1选择在写流程中受控迁移，损坏、缺失或作用域不符均停止。晋升回执升级为v5并冻结当次决策，复验按回执内选择重放，不再读取后来的选择冒充历史。内容差异现在返回当前项目值、文件值、证据位置、是否可继续与精确检查项；硬错误只能换文件，不能用通用“已核对”跳过。前端导入、文档、映射、facts四条异步链加入请求代次，重传或切换后旧响应不能覆盖新状态。

用户界面收敛：三步改为“选择数据→查看导入概况→处理少量疑点”；真实MG接入默认只显示1文件/62表/148788行摘要，62表整体按需展开，每表仍单独折叠；移除误导性的确定性角色徽标；文件角色与差异动作改为自然中文。隔离后端重启后，原批次通过latest API返回200/profile_ready并在浏览器恢复第二步，说明先前一次404是启动窗口的短暂状态，不是持久化丢失；未为该现象添加虚假修复。

集中验证：后端四套120项通过（49条既有警告）；前端状态/渲染/集成/API共411项通过；Vite build、py_compile、git diff --check通过。浏览器实际核验默认折叠、展开后逐表折叠及重启恢复。没有点击“下一步：让系统核对字段”，因为当前会话没有重新核实批准的MiniMax主分析与GLM盲核对两条远程路线；本地Qwen/MTPLX没有启动，也没有把单模型结果称为双核对。

踩坑与纠偏：第一，最初把后端重启后的404怀疑为落盘恢复缺陷，直接检查manifest、SQLite记录、pipeline读取和当前HTTP后确认数据完整，避免用重新上传掩盖问题。第二，初版CAS只做读后写，两个进程仍可能同时通过版本检查；已加跨进程flock并新增并发写入反例，一写成功、一写明确冲突。第三，界面原先一次展开62张表且展示启发式角色，真实浏览器证明对目标用户负担过高；现改为摘要优先和逐表按需查看。
...
### 2026-09-23 W02 当前双路线、草稿原子化与可配置运行参数

现行医学监查角色已按用户要求落为 `cms-router/glm-5.3-flash:high` 主分析与 `ollama-cloud/deepseek-v4.1-flash:high` 独立盲核，均走产品裸 API；本地 Qwen/MTPLX 未选择、未加载，8911持续停止。正确MG首轮两路各147个真实模型分片全部完成，另有8个确定性元数据分片；草稿 `monmapdraft_90eb7a7e2bd34430d94c839a5286` 已从精确主队列原子组装，覆盖62域、1495字段，1494项系统直接采用，仅1项原始疑点，双路实质分歧700字段进入独立二轮复核。
