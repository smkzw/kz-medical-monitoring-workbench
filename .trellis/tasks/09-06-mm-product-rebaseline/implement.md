...
P1续作2026-09-06：持久项目暂停已进入SQLite领取事务；允许在途结果完整落盘、禁止暂停项目领取/过期重领，其他项目独立运行。新增queue状态/暂停/继续API，继续唤醒两模型worker。109项repository/worker/API测试通过。尚未接入用户端按钮和真实长队列，选择性分片重试仍待实现，不称P1完成。
