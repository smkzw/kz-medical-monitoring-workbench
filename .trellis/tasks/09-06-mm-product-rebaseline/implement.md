...
2026-09-06：身份/保守分歧边界、队列持久暂停与原位有界恢复已提交；重复g02通过仓储退役并保留备份、全部候选。显示修复已整合后端比较状态，零变化/首次/加载中分别验证；未知来源核对数不再补成总量。独立队列审阅运行中，浏览器显示交互与柱高测量已通过，截图接口超时导致最终视觉检查仍待补。P1尚未关闭；完整自主补证据裁决属于P2。
...
P1续作2026-09-06：持久暂停/API/用户按钮已接通，ego实际验证离开再进和后端重启保持暂停。分片原位有界自动恢复、idle退出唤醒竞态已修。87个误建g02经在线备份、等价验证后通过仓储退役，571jobs及321候选保留，MG队列尚未开始真实模型恢复。结构预览不再触发父重载，挂载回归通过。零变化/趋势交E03执行节点（详见journal），待终态整合和视觉检查，P1未完成。轻量列表实测显著降低读取成本，完整执行/接受验证保持。
...
### P2当前接线拆分（2026-09-06，准备中）

从用户目标出发，自动补读应减少工程化确认，不应把固定摘要当完整理解。当前run_next仅provider.run初答+一次格式修复；已有retrieve_current_excerpts是后端可用读取器，尚未形成模型主动工具循环。按以下依赖实施：
