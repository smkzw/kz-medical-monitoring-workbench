# Conference Context: mm-0922-review-plan

Created: 2026-09-22 14:33:11 CST
Objective: 独立核查当前医学监查前端到公开Finding/来源导航的完整链路，并挑战0922连续交付规划的遗漏和过度设计；只读审阅，不修改产品或启动真实任务。
...
- Ordinary tasks remain Codex-direct. Chinese labels or Chinese sentence work uses its declared execution route and does not start a conference.
  - This packet uses one Codex-led conference object (`general_single_object`) with no sub-venue chair. Its effective `CST` route chain is `codebuddy/codebuddy-cli/deepseek-v4.1-flash:max -> pi/cursor/default -> pi/openai-codex/gpt-5.6-sol:medium`; the packet branch is recorded at creation and filtered against the actual execution route nodes recorded below. Before a new session, the runner rechecks the Beijing period; an already-started session is never rerouted.
- Every conference role starts with one bounded same-session pass. Codex reviews its quality and may dispatch zero or more targeted follow-up prompts through the same session. A new session is a routing failure unless a primary role failed before a resumable session existed and the documented fallback was activated.

## Execution-Conference Model Deduplication

- Linked execution task: `mm-0922-review-plan`
...
- 2026-09-22 14:33:11 CST: Conference initialized by `hermes_workflow_guard.py init-conference`.
