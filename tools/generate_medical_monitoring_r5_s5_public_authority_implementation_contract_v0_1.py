...
FUTURE_RUNTIME_STATIC_GATE_SPEC = {
    ...
    "producer_acceptance_commands": {
        "static_ast_scan": "PYTHONDONTWRITEBYTECODE=1 python3 -B tools/verify_medical_monitoring_r5_s5_public_authority_implementation_contract_v0_1.py --scan-future-runtime",
        ...
        "contract_verifier": "PYTHONDONTWRITEBYTECODE=1 python3 -B tools/verify_medical_monitoring_r5_s5_public_authority_implementation_contract_v0_1.py",
    },
}
...
def public_api() -> dict[str, Any]:
    ...
    common_inputs = [
        {
            "name": "CurrentJoinContextAuthorityRecord",
            "fields": [
                ...
                ["candidate_field", "str"],
                ["semantic_class", "str"],
                ["authority_value_canonical_json", "str"],
                ["authority_content_hash", "str"],
            ],
            "closed_values": {},
            "resolution": (
                "an internal current-construction join-context record; component_key "
                "selects this record independently of the candidate item and the "
                "canonical JSON value is compared with the candidate authority field"
            ),
            "decorator": "dataclasses.dataclass(frozen=True)",
            "extra_fields_forbidden": True,
        },
        {
            "name": "CurrentJoinContextRegistry",
            "fields": [
                ["records", "tuple[CurrentJoinContextAuthorityRecord, ...]"],
            ],
            "decorator": "dataclasses.dataclass(frozen=True)",
            "extra_fields_forbidden": True,
        },
    ...
    api = {
        "schema": "medical-monitoring-r5-s5-public-authority-public-api-v0.1",
        ...
        "structured_reference_grammar": {
            ...
            "closed_kinds": ["source", "previous", "output", "controlled", "constant", "context"],
            "closed_roots": ["subject_source", "aemh_source", "aemh_previous", "subject_current", "aemh_current", "constant_catalog", "current_join_context"],
            ...
            "relation_key_rule": "each candidate collection is selected by one or two real identity/ref/content-identity fields; authority_reference resolves the candidate/authority-side value while context_reference resolves an independent CurrentJoinContextAuthorityRecord selected by component_key; equality is over canonical JSON values, every provenance reference participates in the output dependency DAG, every output key is strictly prior, every key is distinct from the value terminal except the one frozen SharedSpine self-key, and every composite component is compared",
...
def _binding_catalog(schema: Mapping[str, Any]) -> dict[tuple[str, str], dict[str, Any]]:
    ...
    add("TemporalEvent", ["domain"], [src("mm_r4.visit_schedule", "ActualActivityRecord", "clinical_domain"), src("mm_r1.domain", "TemporalEvent", "event_type"), src("mm_r4.aemh", "SemanticRecord", "role")], join_keys=["event_ref"], derivation="fixed closed mapping table from authoritative clinical_domain/event_type/role", reducer="closed_domain_mapping", closed_mapping="exact eight-domain table; unknown and OTHER forbidden", unavailable_code="PUB_DOMAIN_UNKNOWN")
    add("TemporalEvent", ["subtype"], [src("mm_r4.visit_schedule", "ActualActivityRecord", "activity_kind"), src("mm_r1.domain", "TemporalEvent", "event_type"), out("TemporalEvent", "domain")], join_keys=["event_ref", "domain"], derivation="fixed accepted journey subtype mapping", reducer="closed_subtype_mapping", unavailable_code="PUB_ENUM_UNKNOWN")
    ...
    risk_sources = {
        ...
        "severity": [src("mm_r5.s4_contracts", "S4AcceptedAuthorityAnchor", "severity_authority"), src("mm_r5.s4_contracts", "S4AcceptedRiskIdentity", "severity")],
        "risk_type_zh": [src("mm_r5.s4_contracts", "S4AcceptedRiskIdentity", "domain"), src("mm_r5.s4_contracts", "S4AcceptedRiskIdentity", "domain_zh")],
    ...
    for field_name, sources in risk_sources.items():
        ...
        if field_name == "severity":
            reducer = "accepted_risk_severity_conversion"
            closed_mapping = "critical->critical; high->high; medium->medium; low->low; severe->high; moderate->medium; mild->low; otherwise PUB_ENUM_UNKNOWN"
        elif field_name == "risk_type_zh":
            reducer = "accepted_domain_to_risk_type_zh"
            closed_mapping = "ae->AE风险; mh->MH风险; cm->合并用药风险; ip->试验用药风险; lab_exam->实验室/检查风险; hospital_procedure->住院/操作风险; symptom_efficacy->症状/疗效风险; protocol_compliance->方案依从性风险"
        ...
        for (owner, field_name), binding in sorted(catalog.items()):
            ...
            for reference_index, raw in enumerate(normalized):
                ...
                for join_key_index, authority_reference in enumerate(
                    ...
                    context_component_key = canonical_hash(
                        {
                            "contract": contract,
                            "leaf": leaf,
                    ...
                    key_id = canonical_hash(
                        {
                            ...
                            "context_component_key": context_component_key,
                            "value_terminal": {
                                "owner": value_reference["terminal"]["owner"],
                                "field": value_reference["segments"][-1]["field"],
                            },
                    ...
                    "component_comparison_provenance": [
                        {
                            ...
                            "context_reference": copy.deepcopy(
                                component["context_reference"]
                            ),
...
ERROR_TRIGGER_PATH = {
    ...
    "PUB_CONTRACT_VERSION_MISMATCH": ("projection schema_version or receipt authority_contract_version is not 2026-08-19.1", "/projection/schema_version|/receipt/authority_contract_version"),
...
def invariant_error_matrix() -> dict[str, Any]:
    ...
    return {
        ...
        "cross_contract_invariants": [
            ...
            "subject and AE/MH receipts bind schema_version=2026-08-19.1 and audience_contract_id=contract.s4.1",
...
def _build_fixture_graph(
    ...
    # Materialize the independent current join context after source, previous,
    ...
    nodes[context_root_id] = {
        "type": "CurrentJoinContextRegistry",
    ...
    provisional_graph = {
        "roots": {
            ...
            "join_context": {"node_ref": context_root_id},
        },
        "nodes": nodes,
    }
        ...
        for component in authority["components"]:
            ...
            for ordinal, canonical_value in enumerate(canonical_values):
                ...
                record_id = canonical_hash({
                    "component_key": component["context_component_key"],
                    "value_ordinal": ordinal,
                    "authority_value_canonical_json": canonical_value,
                })
                record_fields = {
                ...
                nodes[record_node_id] = {
                    "type": "CurrentJoinContextAuthorityRecord",
...
def context_markdown(api: Mapping[str, Any], tests: Mapping[str, Any]) -> bytes:
    text = f"""# Task Context: medical_monitoring_r5_s5_public_authority_implementation_contract_20260819
...
Risk: high
State: `IMPLEMENTATION_CONTRACT_FROZEN_FOR_INDEPENDENT_REVIEW`

## Source of truth

- Accepted 10-file public-authority contract snapshot, pinned in `manifest.json`.
- Exact public schemas `{SCHEMA_VERSION}` and parent audience contract `{AUDIENCE_CONTRACT_ID}`.
...
- `source_type_access_paths` freezes bundle-to-external-type reachability; all 272 joins use structured JSON references whose field/one-many-optional/terminal segments and closed field-level `eq/ref_eq/composite_and` predicate AST are independently resolved and executed per candidate item against exact fixture records or pinned Python 3.9 AST dataclasses. Each collection relation key is a real identity/ref/content-identity field distinct from the selected value terminal, except the single declared `SharedSpineBinding.shared_spine_ref` self-key forced by that one-ref record shape; every component freezes separate candidate authority-value and typed current-join-context references, explicit comparison provenance, reducer-input provenance, and target-output provenance. Authority/context reference reflection is forbidden. The historical 1168 abstract-label/field mismatches are frozen as reviewer history and current mismatch count is zero.
...
- All 272 output leaves freeze source dataclass field paths, semantic category, join keys, reducer/closed mapping, cardinality, ordering and fail-closed code. Every predicate component's authority and context reference participates in the output dependency DAG; source/controlled/previous/constant references are immediate authority and every output reference must be strictly prior. The historical v8 defect surface is exactly 207 future/self components across 69 leaves and the current count is zero. Projection and hash leaves use stable source/current identity context rather than the value hash being built. The verifier parses every referenced source class and field from the pinned Python AST and rejects semantic-category substitution.
...
def review_markdown(
    ...
    text = f"""# 医学监查 R5-S5 公共权威实现合同 v0.1
...
- schema/authority version 固定 `{SCHEMA_VERSION}`，audience 固定 `{AUDIENCE_CONTRACT_ID}`。
...
def build_bundle() -> dict[Path, bytes]:
    ...
    manifest_core = {
        "schema": "medical-monitoring-r5-s5-public-authority-implementation-manifest-v0.1",
        "contract_id": CONTRACT_ID,
        "schema_version": SCHEMA_VERSION,
        "audience_contract_id": AUDIENCE_CONTRACT_ID,
