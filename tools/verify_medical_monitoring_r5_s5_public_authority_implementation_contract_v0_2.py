...
EXPECTED_MANIFEST_KEYS = {
    ...
    "source_file_sha256",
    "rejected_v0_1_negative_baseline_sha256",
...
def verify_manifest() -> dict[str, Any]:
    ...
    if manifest["schema"] != (
        "medical-monitoring-r5-s5-public-authority-implementation-manifest-v0.2"
    ):
...
def verify_pins(manifest: Mapping[str, Any]) -> None:
    ...
    semantic = manifest["semantic_authority_delta"]
    if semantic != {
        ...
        "exact_leaf_paths": [
            "TemporalEvent.domain", "TemporalEvent.subtype",
            "TemporalRiskAnchor.risk_type_zh", "TemporalRiskAnchor.severity",
        ],
        "s4_semantic_projection_forbidden": True,
    }:
        fail("accepted semantic-authority delta pin drift")
...
def _execute_structured_selector(
    ...
    for clause, component in zip(clauses, components):
        ...
        expected_authority_reference = component.get("expected_authority_reference")
        if not isinstance(authority_reference, dict) or not isinstance(
            expected_authority_reference, dict
        ):
            fail(f"structured authority reference missing: {row['leaf']}")
        for key_reference in (authority_reference, expected_authority_reference):
            ...
            elif key_reference["kind"] == "expected":
                if (
                    key_reference.get("root") != "independent_expected_authority"
                    ...
                    fail(f"structured independent expected authority ref invalid: {row['leaf']}")
            else:
        ...
        if (
            set(component) != {
                "authority_key", "authority_reference", "expected_authority_reference",
            ...
            or clause["rhs"].get("scope") != "independent_expected_authority"
            ...
            or expected_authority_reference == authority_reference
            or expected_authority_reference.get("kind") != "expected"
            or expected_authority_reference.get("root") != "independent_expected_authority"
    ...
    context_nodes = [
        graph["nodes"][node_id]
        for node_id in sorted(_fixture_reachable(graph, "expected_authority"))
    ...
    for component in components:
        ...
        matching_context = [
            ...
            if node["fields"].get("expected_component_key")
        ...
        for fields in matching_context:
            if (
                ...
                fail(f"independent expected-authority record drift: {row['leaf']}")
            actual_context_values.add(fields["expected_value_canonical_json"])
        if actual_context_values != expected_context_values:
            fail(f"independent relation authority/context drift: {row['leaf']}")
        context_values_by_component[component["expected_component_key"]] = (
    ...
    for record in _structured_fixture_records(reference, graph, api):
        if any(
            ...
            not in context_values_by_component[component["expected_component_key"]]
...
def _execute_join_reducer(
    ...
    if operation == "all_equal":
        ...
        if (
            len(collection_lengths) != len(target_values)
            or target_values != collection_lengths
            or any(not isinstance(value, int) for value in target_values)
        ):
            fail(f"count reducer input/output mismatch: {row['leaf']}")
    elif operation == "sorted_unique":
        expected_scalars = sorted(
            {
        ...
        if row["leaf"] == "SourceRevisionContentPair.locator_refs":
            expected_scalars = sorted({
                item
                for value in selected_by_reference[0]
                for item in (value if isinstance(value, list) else [value])
            })
            partitioned = [item for target in target_values for item in target]
            if (
                sorted(partitioned) != expected_scalars
                or len(partitioned) != len(set(partitioned))
            ):
                fail(f"sorted-unique revision partition mismatch: {row['leaf']}")
            return
        for target in target_values:
            if isinstance(target, list):
                ...
                if (
                    row["reducer"].startswith("sorted_unique")
                    and all(not isinstance(value, (dict, list)) for value in target)
                    and target != expected_scalars
                ):
                    fail(f"sorted-unique reducer exact set mismatch: {row['leaf']}")
            elif isinstance(target, dict):
                hash_fields = [
                    value for key, value in target.items()
                    if key.endswith(("_content_hash", "_hash"))
                ]
                if not hash_fields or not set(hash_fields).intersection(selected):
                    fail(f"sorted-unique object authority mismatch: {row['leaf']}")
            elif (
                row["reducer"] == "exactly_one_or_sorted_unique"
                and target not in selected
            ):
                fail(f"sorted-unique scalar authority mismatch: {row['leaf']}")
    ...
    elif operation == "semantic_authority_exact_sequence":
        ...
        if canonical_target != canonical_selected:
            fail(f"semantic authority exact sequence mismatch: {row['leaf']}")
...
def _validate_fixture_graph(graph: Mapping[str, Any], api: Mapping[str, Any]) -> None:
    ...
    context_id = graph["roots"]["expected_authority"]["node_ref"]
    ...
    if graph["nodes"][context_id]["type"] != "IndependentExpectedJoinAuthorityRegistry":
        fail("fixture independent expected-authority root type mismatch")
    ...
    for root_name in ("source", "previous", "expected_candidate", "expected_authority"):
...
def verify_redesign_negative_controls(
    ...
    component["expected_authority_reference"] = copy.deepcopy(
    ...
    for key, authority in all_terminal_copy_api[
        ...
        for component in authority["components"]:
            ...
            component["expected_authority_reference"] = copy.deepcopy(value_reference)
            component["join_key"] = (
            ...
            new_key = canonical_hash({
                ...
                "expected_authority_reference": value_reference,
                "value_terminal": component["value_terminal"],
            })
            component["authority_key"] = new_key
            clause = next(
    ...
    future_component["expected_authority_reference"] = copy.deepcopy(later_reference)
    ...
    future_component["authority_key"] = canonical_hash({
        ...
        "expected_authority_reference": later_reference,
    ...
    reflected_component["expected_authority_reference"] = copy.deepcopy(
