# Task Context: medical_monitoring_ai_detail_identity_guard_20260806

Created: 2026-08-06 04:47:41
Objective: 补齐日常医学监查运行详情响应的 project_id/run_id 身份守卫，防止跨项目或跨运行详情进入当前面板；仅做离线前端合同、测试与记录，不启动运行时
...
- `services/api/app/monitoring_daily_run_router.py` (`GET /api/projects/{project_id}/monitoring/daily-runs/{run_id}`)
...
- In scope: strict client-side normalization of the existing detail response identity, panel call-site binding, focused/static tests, and durable evidence records.
...
- A detail response is accepted only when the expected project/run identity is present and the top-level project, nested run project, and nested run ID all match.
...
- 2026-08-06 04:47:41: Task initialized by `tools/hermes_workflow_guard.py init-task`.
