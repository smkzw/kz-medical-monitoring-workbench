# Task Context: medical_monitoring_ai_readiness_identity_guard_20260806

Created: 2026-08-06 04:57:18
Objective: 补齐日常医学监查启动就绪响应的 project_id/batch_id 身份守卫，防止跨项目或跨批次 readiness 进入当前面板；仅做离线前端合同、测试与记录，不启动运行时
...
- `services/api/app/monitoring_daily_run_router.py` (`GET /api/projects/{project_id}/monitoring/daily-runs/readiness`)
- `services/api/app/monitoring_daily_run_service.py` (`start_readiness`)
...
- In scope: strict client-side normalization of the existing readiness response identity and minimum readiness shape, both panel call-site bindings, focused/static tests, and durable evidence records.
...
- Readiness is accepted only when expected project/batch identity is present and exact response identity matches; `ready` and `next_action` remain strict typed gate inputs.
- Missing or mismatched identity/shape fails closed with an explicit user-facing error and cannot enable the start action.
...
- 2026-08-06 04:57:18: Task initialized by `tools/hermes_workflow_guard.py init-task`.
