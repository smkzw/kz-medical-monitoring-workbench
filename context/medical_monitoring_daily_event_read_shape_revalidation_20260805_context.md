# Task Context: medical_monitoring_daily_event_read_shape_revalidation_20260805

Created: 2026-08-05 07:06:54
Objective: Harden persisted monitoring daily-run event read shape without runtime activation
...
- `services/api/app/monitoring_daily_run_repository.py`, persisted event reader
  `_event`, and its existing daily-run event tests.
...
- In scope: strict fail-closed validation of persisted event counters, payload
  text/JSON shape, and timestamps; one focused tamper regression; non-runtime
  compile/test evidence and task records.
- Out of scope: step/snapshot/business behavior changes, schema migrations,
...
- Persisted event reads reject float/string/bool counters and malformed
  payload/timestamp shapes without coercion.
...
- 2026-08-05 07:06:54: Task initialized by `tools/hermes_workflow_guard.py init-task`.
