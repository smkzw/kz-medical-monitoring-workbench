...
- Runner-managed output path: `runs/codex_medical_monitoring_ai_router_hash_shape_revalidation_20260805.md`. Never invoke a
  write/edit tool on this report path; return the complete report in your final
  response and let the runner persist it.

Read these files only:
- `context/medical_monitoring_ai_router_hash_shape_revalidation_20260805_context.md`
...
Present persisted profile/input digests must not be accepted after `strip()` or
`lower()` rewriting. Request models must require exact lowercase 64-hex
SHA-256 values; status/legacy/adoption comparisons must preserve raw values or
fail closed. Keep valid mapping status/adoption behavior compatible and do not
widen scope.
...
Output schema:
1. `# Codex Direct Task: medical_monitoring_ai_router_hash_shape_revalidation_20260805`
2. `## Boundary Check`
3. `## Direct Work`
