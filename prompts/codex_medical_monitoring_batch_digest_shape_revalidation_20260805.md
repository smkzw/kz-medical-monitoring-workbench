...
- Runner-managed output path: `runs/codex_medical_monitoring_batch_digest_shape_revalidation_20260805.md`. Never invoke a
  write/edit tool on this report path; return the complete report in your final
  response and let the runner persist it.

Read these files only before editing:
- `context/medical_monitoring_batch_digest_shape_revalidation_20260805_context.md`
...
directly. Ensure `verify_derived_snapshot()` fails closed on padded, uppercase,
non-hex, short, or non-string `source_content_sha256` instead of lower/strip
rewriting it, while preserving canonical success, mismatch, idempotency, source
promotion, and batch-state behavior. Use the smallest coherent code/test patch
and do not alter unrelated batch/parser/mapping semantics.
...
Output schema:
1. `# Codex Direct Task: medical_monitoring_batch_digest_shape_revalidation_20260805`
2. `## Boundary Check`
3. `## Direct Work`
