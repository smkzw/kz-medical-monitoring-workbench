...
- Runner-managed report path: `runs/execution/mw_prefill_deepseek_prod_exec_20260720/worker_01.md`. Never invoke write/edit tools
...
Task:
Execute only this assigned work item: Implement one bulk production AI prefill adapter and wire it outside SQLite write transactions; preserve existing deterministic fallback and exact-fact evidence gates.

Read `context/mw_prefill_deepseek_prod_20260720_context.md` and the source files
listed in the shared execution context before editing. Your exclusive write set
is the `worker_01` source set declared there. Do not edit tests, frontend,
records, runtime databases or task logs.

Implementation requirements:
- use one bulk structured DeepSeek request, not one call per field;
- verify the actual returned model identity is `deepseek-v4-pro`;
- merge only valid AI candidates into the deterministic package;
- preserve deterministic full and partial fallback;
- keep the external call outside any SQLite write transaction;
- propose an English ClinicalTrials.gov condition term as a candidate without
  silently confirming it;
- keep exact facts blocked without registered direct evidence;
- preserve immutable audit/source metadata and optimistic conflict behavior.
...
Output schema:
1. `# Execution Output: mw_prefill_deepseek_prod_exec_20260720 - worker_01`
