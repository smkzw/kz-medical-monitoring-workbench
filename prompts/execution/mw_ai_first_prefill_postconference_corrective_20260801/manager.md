...
- Runner-managed report path: `runs/execution/mw_ai_first_prefill_postconference_corrective_20260801/manager.md`. Never invoke write/edit tools
...
All three workers execute serially in the shared workspace. Treat their reports as claimed
evidence only: inspect current source after all workers, verify that shared-file edits were not
lost, run the complete focused authoring/prefill suite, and map every chair recheck to direct
evidence. Do not start services or a browser. If any P2-P4 contract is incomplete, return a
precise same-session rerun request; do not soften it into a recommendation.
...
Output schema:
1. `# Execution Output: mw_ai_first_prefill_postconference_corrective_20260801 - finite_code_manager_cursor`
