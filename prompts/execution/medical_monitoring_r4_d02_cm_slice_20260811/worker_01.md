...
- Runner-managed report path: `runs/execution/medical_monitoring_r4_d02_cm_slice_20260811/worker_01.md`. Never invoke write/edit tools
...
Task:
Execute only this assigned work item: Serial shared-surface prerequisite: add RiskDomainUnitResult, neutral priority/identity accessors and CrossDomainEvidenceRef in contracts.py; adapt AEMHUnitResult/lifecycle with no D01 behavior change; run D01 224 before proceeding

You are the only shared-surface writer. Create or modify only the files listed for `worker_01` in the execution context. Implement the exact public Protocol fields and canonical evidence/identity helpers from the frozen D02 contract; move neutral constants/accessors to `contracts.py`, re-export compatibility names from `aemh.py`, and make `lifecycle.py` depend only on the neutral Protocol/accessors. Preserve D01 semantics exactly. Add only `tests/test_shared_domain_protocol.py` if a new test is needed. Run the original D01 suite excluding that new file and require exactly `224 passed`, then run the new file separately. Do not begin CM implementation.
...
Output schema:
1. `# Execution Output: medical_monitoring_r4_d02_cm_slice_20260811 - worker_01`
