...
- Runner-managed report path: `runs/execution/medical_monitoring_r4_d02_cm_slice_20260811/worker_04.md`. Never invoke write/edit tools
...
Task:
Execute only this assigned work item: Implement synthetic fixtures/challenge matrix 1-30, N-to-N+1 lifecycle and adjacent regression evidence in cm_fixtures.py/tests

Do not start unless the execution context or Codex-provided continuation records that the worker_01 shared-prerequisite gate passed and the D02 engine/projection files are present as stable read dependencies. Write only new `src/mm_r4/cm_fixtures.py` and new `tests/test_cm_challenge_matrix.py`. Turn every numbered frozen challenge case 1-30 into executable synthetic evidence with explicit expected L1/L2/L3, identity, coverage, Query, projection, or lifecycle assertions as applicable. Include an N-to-N+1 update proving immutable historical completion, stable event identity across locator/snapshot revision, versioned lineage, no duplicate D01/D02 candidates/risks/Queries, and deterministic replay. Do not use real projects or external medical data.
...
Output schema:
1. `# Execution Output: medical_monitoring_r4_d02_cm_slice_20260811 - worker_04`
