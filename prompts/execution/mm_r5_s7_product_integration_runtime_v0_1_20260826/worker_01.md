...
- Task id: `mm_r5_s7_product_integration_runtime_v0_1_20260826`
- Role id: `worker_01`
...
- Runner-managed report path: `runs/execution/mm_r5_s7_product_integration_runtime_v0_1_20260826/worker_01.md`. Never invoke write/edit tools
  to create or update this report file. Return the complete report in your
  final assistant response; the runner persists it. Do not create sibling
  process files.

Initial read set:
- `context/mm_r5_s7_product_integration_runtime_v0_1_20260826_execution_context.md`
- `plans/codex_execution_mm_r5_s7_product_integration_runtime_v0_1_20260826.md`

The initial read set is not a blanket prohibition on additional tool calls or evidence. If more context is required, obtain it with the available tools, explain why, and record what was read or changed.

Objective:
按已接受的R5-S7合同与闭合allowlist实现产品最小接入：新增GET-only R5后端router/adapter与前端R5页面/adapter/route/CSS/fixtures，做最小App/main/read-action接线，完成聚焦和相邻离线回归及production build；本execution不启动8911/5174/浏览器，不运行真实项目/模型，不修改医学写作。

Task:
Execute only this assigned work item: 后端实现：仅写合同4.2/4.3精确allowlist，提供三条GET-only R5 API、synthetic隔离projection、完整identity/digest/read-only envelope、fail-closed参数与测试；最小修改main.py/read-action contract。
...
Output schema:
1. `# Execution Output: mm_r5_s7_product_integration_runtime_v0_1_20260826 - worker_01`
