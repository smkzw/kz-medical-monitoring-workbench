...
- Runner-managed report path: `runs/conference/mm_r7_slice09b_implementation_acceptance_20260830/general_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r7_slice09b_implementation_acceptance_20260830_conference_context.md`
- `plans/codex_main_venue_mm_r7_slice09b_implementation_acceptance_20260830.md`
...
Objective:
独立审查 R7 Slice-09B 项目格式识别、staging-only 迁移、回滚恢复、只读兼容与中文产品投影实现是否完整满足冻结 v0.2 合同；逐项报告 P0-P4，重点核查路径边界、恢复入口、指纹漂移、只读强制、进度真实性、R1/09A 相邻回归及医学写作隔离；不得修改源码或宣称最终验收。
...
Output schema:
1. `# Conference Participant Output: mm_r7_slice09b_implementation_acceptance_20260830 - general_single_object`
2. `## Boundary Check`
