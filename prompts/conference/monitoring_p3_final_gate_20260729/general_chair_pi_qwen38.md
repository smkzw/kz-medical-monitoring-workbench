...
- Runner-managed report path: `runs/conference/monitoring_p3_final_gate_20260729/general_chair_pi_qwen38.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `AGENTS.md`
- `context/monitoring_p3_final_gate_20260729_conference_context.md`
- `plans/codex_main_venue_monitoring_p3_final_gate_20260729.md`
- `runs/conference/monitoring_p3_final_gate_20260729/scheduled_codebuddy_hy3.md`
- `runs/conference/monitoring_p3_final_gate_20260729/general_codebuddy_deepseek_pro.md`
...
Objective:
只读对抗审阅 P3 严格规则生命周期最终阻断修复：核对 gold case 不可变来源绑定、exact rule revision、影子案例集冻结与确认/发布复验、canonical field lineage、legacy lifecycle v0 生产隔离；只报告可复现 P0/P1 或明确通过，不修改文件

Task:
Review the two available participant outputs and independently inspect only the
changed P3 source/tests needed to adjudicate disagreements. Produce a release
gate: reproducible P0/P1 blockers with exact locators and minimal fixes, or
explicit PASS. Treat suggestions outside the user's seven requirements as
residual risk rather than expanding scope. Do not claim Codex-owned final
authority.

Codex has isolated one additional release-gate question: at
`monitoring_protocol_rule_service.py:468-495`, can a repository-registered
negative gold case with `evaluation_state=indeterminate`, `matched=False`, and
otherwise ready source evidence be marked passed merely because
`expected_match=False`? Adjudicate whether strict gold evidence must require
`evaluation_state` to be exactly `true` or `false`, and propose the smallest
test/fix if this is a blocker. Also verify the claimed AE-03 test gap against
`tests/test_monitoring_real_listing_shadow_cases.py:1964-1966`.
...
Output schema:
1. `# Sub-Venue Review: monitoring_p3_final_gate_20260729 - general_chair_pi_qwen38`
2. `## Inputs Reviewed`
