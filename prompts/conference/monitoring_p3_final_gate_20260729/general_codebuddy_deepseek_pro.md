...
Hard boundaries:
- Work only inside the current workbench and respect the declared read set.
...
- Do not write the runner-managed report path `runs/conference/monitoring_p3_final_gate_20260729/general_codebuddy_deepseek_pro.md`; return the complete report for the runner.
...
Task:
Run an independent adversarial, read-only pass over only the source and test
files listed in the conference context. Test the seven acceptance requirements
against direct constructor/repository calls, post-confirm evidence mutation,
SQLite legacy rows, exact revision filtering and publish transaction ordering.
Do not look at other participant outputs. Report only reproducible P0/P1
findings with file:line evidence and a minimal repair, or an explicit pass with
the attack surfaces checked and residual risk.
...
Output schema:
1. `# Conference Participant Output: monitoring_p3_final_gate_20260729 - general_codebuddy_deepseek_pro`
2. `## Boundary Check`
