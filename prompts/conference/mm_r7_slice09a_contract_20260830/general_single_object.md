...
- Runner-managed report path: `runs/conference/mm_r7_slice09a_contract_20260830/general_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r7_slice09a_contract_20260830_conference_context.md`
- `plans/codex_main_venue_mm_r7_slice09a_contract_20260830.md`
...
Objective:
独立挑战并冻结 R7 Slice-09A 项目备份、恢复、导出与导入合同；核查最小范围、包闭包、一致快照、原子恢复、故障矩阵、中文用户语义与相邻回归，不实施产品代码。

Task:
Run an independent whole-workflow pass for your assigned role. Do not look at other participant outputs. Audit the current source sufficiently to challenge the contract, but do not modify any file. Return a verdict of `ACCEPT_CONTRACT_V0_1`, `ACCEPT_WITH_REQUIRED_CORRECTIONS`, or `REJECT`; enumerate P0-P4 defects and exact replacement clauses. Resolve or explicitly bound all six Section 14 decision points. Produce findings, risks, verification needs, and questions for Codex.
...
Output schema:
1. `# Conference Participant Output: mm_r7_slice09a_contract_20260830 - general_single_object`
2. `## Boundary Check`
