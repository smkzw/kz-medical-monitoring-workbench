...
Hard boundaries:
- Work only inside the current workspace (`.`); do not traverse to any parent or sibling directory.
...
- Runner-managed report path: `runs/conference/medical_monitoring_r4_d04_contract_acceptance_20260812/general_pi_qwen38.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `AGENTS.md`
- `context/medical_monitoring_r4_d04_contract_acceptance_20260812_conference_context.md`
- `plans/codex_main_venue_medical_monitoring_r4_d04_contract_acceptance_20260812.md`
...
Objective:
独立挑战并验收R4-D04入排、方案要求与潜在方案偏离合成离线合同

Task:
Act as the independent clinical/protocol-review challenger. Verify that the draft is medically conservative, protocol-version aware, faithful to official criterion numbering and implementable without converting model output or aggregate IE/DV fields into evidence. Challenge exact threshold/unit/date/retest/waiver/investigator-judgment semantics, pre-enrollment IE versus post-enrollment deviation boundaries, Query wording and the prohibition on formal PD claims. Do not look at other participant outputs. Return an explicit `ACCEPT` or `REVISE`; for every blocker cite the exact section and give replacement wording or a concrete synthetic case.
...
Output schema:
1. `# Conference Participant Output: medical_monitoring_r4_d04_contract_acceptance_20260812 - general_pi_qwen38`
2. `## Boundary Check`
