...
- Runner-managed report path: `runs/conference/mm_r7_slice09b_contract_acceptance_20260830/general_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r7_slice09b_contract_acceptance_20260830_conference_context.md`
- `plans/codex_main_venue_mm_r7_slice09b_contract_acceptance_20260830.md`
...
Objective:
独立审阅 R7 Slice-09B 项目格式升级、兼容与恢复合同 v0.1 及其三项执行证据，逐条挑战来源版本边界、升级前恢复点、staging-only、marker-last、跨库切换/回滚、崩溃重试、旧项目只读和中文 DTO。输出 P0-P4 问题及是否可冻结；不得修改产品源码，不运行服务、模型或真实项目。
...
Output schema:
1. `# Conference Participant Output: mm_r7_slice09b_contract_acceptance_20260830 - general_single_object`
2. `## Boundary Check`
