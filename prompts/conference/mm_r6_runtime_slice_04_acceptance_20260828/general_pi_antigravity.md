...
- Runner-managed report path: `runs/conference/mm_r6_runtime_slice_04_acceptance_20260828/general_pi_antigravity.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r6_runtime_slice_04_acceptance_20260828_conference_context.md`
- `plans/codex_main_venue_mm_r6_runtime_slice_04_acceptance_20260828.md`
...
Objective:
独立审阅 R6 第四纵切 synthetic/offline ModeContract、ModeOutput、daily 四输出与结构化 Query 草稿是否满足 slice-04 合同和权威 §8/contract.json；重点寻找 Run/mode/carry-forward、output identity/eligibility、authority/numeric、Query 依据+发现+行动项及 sent/closed/PD 边界失败开放；核对 460 回归与 9 宫格。不得修改文件或接受产品/真实项目/医学范围。
...
Output schema:
1. `# Conference Participant Output: mm_r6_runtime_slice_04_acceptance_20260828 - general_pi_antigravity`
2. `## Boundary Check`
