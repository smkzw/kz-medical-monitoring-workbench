...
- Runner-managed report path: `runs/conference/mm_r7_slice_04_progress_acceptance_20260828/general_pi_antigravity.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r7_slice_04_progress_acceptance_20260828_conference_context.md`
- `plans/codex_main_venue_mm_r7_slice_04_progress_acceptance_20260828.md`
...
Objective:
对 R7 Slice-04 耐久运行进度实现进行独立只读验收：逐条核对冻结契约、身份隔离、零I/O边界、幂等与回退阻断、post_lock冻结、中文产品投影、测试与证据真实性；仅报告可复现缺陷，不修改文件，不启动服务/模型/真实项目，不触碰医学写作子系统。
...
Output schema:
1. `# Conference Participant Output: mm_r7_slice_04_progress_acceptance_20260828 - general_pi_antigravity`
2. `## Boundary Check`
