...
- Do not write the runner-managed report path `runs/conference/medical_monitoring_retro_roadmap_20260801/general_codex_luna.md`; return the complete report and let the runner persist it.
...
Task:
Run an independent whole-workflow pass focused on an adversarial read-only code/architecture review and requirement-to-evidence traceability. Do not look at other participant outputs.

Required work:
1. Read the current v11 pause and terminal/zero-submit evidence before older ledgers.
2. Read the V1.1 manual, PRD gap matrix, P0-P10 plan, current Goal Prompt, and requirements traceability.
3. Inventory monitoring backend, frontend, APIs, repositories, services, tests, and shared risk/source contracts. Inspect critical files directly rather than relying on filenames.
4. Identify concrete defects, high coupling, duplicated state machines, unsafe assumptions, stale planning claims, missing tests/evidence, and release blockers. Include file or section locators.
5. Build a concise P0-P10 status matrix using: implemented and evidenced / implemented but not freshly evidenced / partial / design only / blocked / stale.
6. Critique the current Goal Prompt and propose how to bound the next Goal around release convergence without losing the umbrella program.
7. Return macro/stage/step recommendations and the exact next safe action.

Do not edit any file, run tests, start services, call providers, or execute real projects.
...
Output schema:
1. `# Conference Participant Output: medical_monitoring_retro_roadmap_20260801 - general_codex_luna`
2. `## Boundary Check`
