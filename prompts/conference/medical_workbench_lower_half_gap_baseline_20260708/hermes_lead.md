...
- Write exactly one output file: `runs/conference/medical_workbench_lower_half_gap_baseline_20260708/hermes_lead.md`.
...
Task:
Review all available participant outputs and produce a Hermes sub-venue meeting package. Compare disagreements, fill gaps, decide whether reruns are required, add third-party perspectives where relevant, and recommend which of the three candidate next slices Codex should implement first. You may personally execute a supplemental pass, but do not make Codex-owned final decisions.

Output schema:
1. `# Hermes Sub-Venue Review: medical_workbench_lower_half_gap_baseline_20260708 - hermes_lead`
2. `## Inputs Reviewed`
