...
- Write exactly one output file: `runs/conference/medical_workbench_lower_half_gap_baseline_20260708/participant_qwen_plus.md`.
- After reading the allowed files, your first concrete action must be to prepare and write that output file. Do not stop after title generation, session naming, or a short summary.
- If the file-writing tool is unavailable or fails, print the complete markdown output body to stdout with the same headings so Codex can mark the route as either usable or failed.
...
Output schema:
1. `# Conference Participant Output: medical_workbench_lower_half_gap_baseline_20260708 - participant_qwen_plus`
2. `## Boundary Check`
