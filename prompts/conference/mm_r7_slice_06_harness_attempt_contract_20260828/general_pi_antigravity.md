...
- Runner-managed report path: `runs/conference/mm_r7_slice_06_harness_attempt_contract_20260828/general_pi_antigravity.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r7_slice_06_harness_attempt_contract_20260828_conference_context.md`
- `plans/codex_main_venue_mm_r7_slice_06_harness_attempt_contract_20260828.md`
...
Objective:
独立审阅并挑战 R7 Slice-06 harness capability attempt、有限重试、恢复与诚实 session/cancel 语义合同；只读源码与合同，不修改文件，不调用模型服务或真实项目，不启动产品服务。输出必须逐项回答四个待会商问题，列出 P0-P4 缺陷及可执行修订建议，并明确 ACCEPT 或 REVISE。
...
Output schema:
1. `# Conference Participant Output: mm_r7_slice_06_harness_attempt_contract_20260828 - general_pi_antigravity`
2. `## Boundary Check`
