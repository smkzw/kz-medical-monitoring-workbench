...
- Runner-managed report path: `runs/conference/mm_r7_slice_03_product_mount_acceptance_20260828/general_pi_antigravity.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r7_slice_03_product_mount_acceptance_20260828_conference_context.md`
- `plans/codex_main_venue_mm_r7_slice_03_product_mount_acceptance_20260828.md`
...
Objective:
独立审阅 R7 Slice-03 产品挂载是否符合冻结合同：检查产品路由、主应用最小接线、测试与证据；重点寻找 bootstrap 唯一创建边界、项目/Run 身份与自动 scope、中文局部错误、连接关闭、MTPLX medium 默认及显式 DeepSeek V4 Flash max 接入能力中的 P0-P2 缺陷。禁止修改任何文件、启动服务、调用模型或真实项目；输出可定位发现与接受/拒绝建议。
...
Output schema:
1. `# Conference Participant Output: mm_r7_slice_03_product_mount_acceptance_20260828 - general_pi_antigravity`
2. `## Boundary Check`
