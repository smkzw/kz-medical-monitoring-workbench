...
- Runner-managed report path: `runs/conference/mw_company_protocol_template_authority_20260719/general_chair_grok45.md`. Never invoke write/edit tools
...
- `runs/conference/mw_company_protocol_template_authority_20260719/general_aishuo_cms.md`
- `runs/conference/mw_company_protocol_template_authority_20260719/general_opencode_deepseek_flash.md`
...
Task:
Review all available participant outputs and the evidence matrix, then produce a sub-venue meeting package. Reconcile a concrete company-first synopsis/protocol contract, plug-in selection algorithm, migration path from the current M11 tree, compatibility risks, and test plan. Explicitly reject any proposal that turns one project's exact labels into an unconditional universal tree or that lets M11 display titles override company practice. Start with one bounded synthesis pass in this session. Codex may send one or more follow-up prompts in the same session when the first pass leaves evidence gaps, contradictions, unresolved reviewer objections, or a justified rerun need. Do not claim Codex-owned final authority.
...
Output schema:
1. `# Hermes Sub-Venue Review: mw_company_protocol_template_authority_20260719 - general_chair_grok45`
2. `## Inputs Reviewed`
