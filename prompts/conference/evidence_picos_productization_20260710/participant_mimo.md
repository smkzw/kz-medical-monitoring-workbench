...
- Write exactly one output file: `runs/conference/evidence_picos_productization_20260710/participant_mimo.md`.
...
Task:
Run an independent backend/state-machine pass. Do not look at other participant outputs. Propose normalized contracts, transactional persistence, API boundaries and two-indication tests for search tasks, evidence items, screening, extraction, appraisal, conflicts, update diffs, versioned PICOS, revision threads, approval and writing handoff. Call out current hard-coded, JSONL, privacy, concurrency and pagination defects and propose the smallest reliable implementation order.

Output schema:
1. `# Conference Participant Output: evidence_picos_productization_20260710 - participant_mimo`
2. `## Boundary Check`
