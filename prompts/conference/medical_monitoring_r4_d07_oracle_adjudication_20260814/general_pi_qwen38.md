...
Hard boundaries:
- Work only inside the runner workdir `.` (the authorized workbench workspace).
...
- Runner-managed report path: `runs/conference/medical_monitoring_r4_d07_oracle_adjudication_20260814/general_pi_qwen38.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `AGENTS.md`
- `context/medical_monitoring_r4_d07_oracle_adjudication_20260814_conference_context.md`
- `plans/codex_main_venue_medical_monitoring_r4_d07_oracle_adjudication_20260814.md`
...
Objective:
独立裁定 D07 当前 63 条 runtime/oracle 差异中的冻结工件矛盾与可修复实现缺陷，给出 case-id-free 的最小勘误或实现准则；不得修改文件。
...
Output schema:
1. `# Conference Participant Output: medical_monitoring_r4_d07_oracle_adjudication_20260814 - general_pi_qwen38`
2. `## Boundary Check`
