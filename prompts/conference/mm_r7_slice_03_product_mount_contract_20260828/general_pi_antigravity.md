...
- Runner-managed report path: `runs/conference/mm_r7_slice_03_product_mount_contract_20260828/general_pi_antigravity.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r7_slice_03_product_mount_contract_20260828_conference_context.md`
- `plans/codex_main_venue_mm_r7_slice_03_product_mount_contract_20260828.md`
...
Objective:
独立审阅 R7 Slice-03 产品挂载与项目工作区合同草案：检查现有 main.py/监查路由接线、项目身份与工作区解析、每请求连接生命周期、中文错误、MTPLX 默认与显式 DeepSeek、零导入业务写入、医学写作保护和受限实现边界；只读，不修改文件、不启动服务或真实项目
...
Output schema:
1. `# Conference Participant Output: mm_r7_slice_03_product_mount_contract_20260828 - general_pi_antigravity`
2. `## Boundary Check`
