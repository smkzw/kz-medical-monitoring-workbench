...
- Runner-managed report path: `runs/conference/mw_ai_first_corpus_prefill_runtime_challenge_20260801/general_chair_pi_qwen38.md`. Never write that report path with tools; return the complete report and let the runner persist it.
...
- `runs/conference/mw_ai_first_corpus_prefill_runtime_challenge_20260801/general_codex_luna.md`
- `runs/conference/mw_ai_first_corpus_prefill_runtime_challenge_20260801/general_pi_deepseek_flash.md`
...
- Compare participants against the conference context and the final
  runtime/context/run/review/metrics records.
- Resolve disagreements on exact-fact quarantine, deterministic extraction,
  one-event lineage, repeated-source rendering, and `当前0` wording.
- Reject severity labels without an exact code/SQLite locator and plausible
  user consequence.
- Recommend READY only when no P0-P4 issue remains. Otherwise provide a
  smallest bounded correction and exact recheck set.
- Read-only only: do not edit files, run tests, control services/UI, or invoke
  OCR/translation/model generation.
...
Output schema:
1. `# Sub-Venue Review: mw_ai_first_corpus_prefill_runtime_challenge_20260801 - general_chair_pi_qwen38`
2. `## Inputs Reviewed`
