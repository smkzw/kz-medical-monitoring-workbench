...
- Runner-managed report path: `runs/conference/mw_dynamic_protocol_modules_20260719/general_aishuo_cms.md`. Never invoke write/edit tools
...
- Read the authority matrix and current implementation named in the conference
  context. You may read the explicitly listed external DOCX/PDF references
  read-only.
- Think as a senior Chinese clinical protocol writer and medical lead. Build a
  concrete decision-to-artifact propagation matrix rather than a generic
  software proposal.
- Audit how a medical manager confirms or overrides AI-prefilled design
  choices and how those choices update synopsis, body chapters, statistical
  methods, schedule of activities, evidence requirements and candidate text.
- Pay special attention to negative language, unknown/deferred decisions,
  design changes after user editing, and the difference between omitting a
  chapter and retaining it with “不适用”.
...
Output schema:
1. `# Conference Participant Output: mw_dynamic_protocol_modules_20260719 - general_aishuo_cms`
2. `## Boundary Check`
