...
- Runner-managed report path: `runs/conference/mw_dynamic_protocol_modules_20260719/general_opencode_deepseek_flash.md`. Never invoke write/edit tools
...
- Read the authority matrix and current implementation named in the conference
  ...
  read-only.
- Audit the proposed dynamic-module mechanism from backend-contract,
  deterministic rule-engine, state-transition, migration, conflict-detection
  and cross-project testing perspectives.
- Give file/function-level findings and propose a compact machine-readable
  applicability model. LLM output may propose values but must not be the sole
  mechanism deciding whether sections exist.
- Cover negative language, unknown/deferred decisions, user overrides and
  propagation into synopsis, body, statistics, schedule and AI candidate
  generation.
...
Output schema:
1. `# Conference Participant Output: mw_dynamic_protocol_modules_20260719 - general_opencode_deepseek_flash`
2. `## Boundary Check`
