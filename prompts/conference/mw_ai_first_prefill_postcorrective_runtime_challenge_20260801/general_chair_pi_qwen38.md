...
- Runner-managed report path: `runs/conference/mw_ai_first_prefill_postcorrective_runtime_challenge_20260801/general_chair_pi_qwen38.md`. Never write that report path with tools; return the complete report and let the runner persist it.
...
Independently inspect the conference source packet before synthesizing. Do not
accept participant or manager confidence as evidence. Reconcile every claimed
P0-P4 against exact source/test/runtime locators, especially the user-edit
channel versus card adoption, empty recommendation frontend behavior,
post-dispatch reservation failure windows, catalog revision identity, and the
r6 preview/isolation proof. For each disagreement, state which claim is
supported and why.

Your final sub-venue verdict must be exactly one of:
`READY_NO_P0_P4`, `NOT_READY`, or `EVIDENCE_BLOCKED`.
`READY_NO_P0_P4` requires an explicit no-issue scope for all seven challenge
areas and no unresolved P0-P4 from either participant. Any defect must include
severity, user impact, smallest correction, and exact recheck. Do not modify
product/runtime files, start services, click the browser, or call a product
model.
...
Output schema:
1. `# Sub-Venue Review: mw_ai_first_prefill_postcorrective_runtime_challenge_20260801 - general_chair_pi_qwen38`
2. `## Inputs Reviewed`
