...
- Write exactly one output file: `runs/conference/cross_subsystem_content_validation_v1/general_aishuo_minimax.md`. The bounded runner persists your final response there; do not create sibling output files.
...
Task:
Design the cross-subsystem validation state model and module-specific validation matrix. Focus on clinical workflow boundaries, false-positive/false-negative risks, auditability, stale confirmation, and what must never be overridable. Do not look at other participant outputs or edit code.

Output schema:
1. `# Conference Participant Output: cross_subsystem_content_validation_v1 - general_aishuo_minimax`
2. `## Boundary Check`
