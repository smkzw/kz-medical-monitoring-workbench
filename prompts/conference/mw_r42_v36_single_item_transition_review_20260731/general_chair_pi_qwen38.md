...
- Runner-managed report path: `runs/conference/mw_r42_v36_single_item_transition_review_20260731/general_chair_pi_qwen38.md`. Never write that report path with tools; return the complete report and let the runner persist it.
...
- `runs/conference/mw_r42_v36_single_item_transition_review_20260731/general_codex_luna.md`
- `runs/conference/mw_r42_v36_single_item_transition_review_20260731/general_codex_luna_recheck.md`
- `runs/conference/mw_r42_v36_single_item_transition_review_20260731/general_pi_deepseek_flash.md`
...
The two original participant reports are preserved as the initial
`NOT READY` evidence. Their corresponding `_recheck.md` files are the
same-session post-remediation verdicts and both report `READY`. Compare the
initial findings, actual repair, and recheck evidence; do not treat the stale
first-pass verdict as the current implementation verdict. The intentional
terminal `model_call_outcome_unknown_after_restart` state is Codex's recorded
fail-closed decision when a committed dispatch has neither a provider-side
idempotency/result lookup nor a provable persisted output.
...
Output schema:
1. `# Sub-Venue Review: mw_r42_v36_single_item_transition_review_20260731 - general_chair_pi_qwen38`
2. `## Inputs Reviewed`
