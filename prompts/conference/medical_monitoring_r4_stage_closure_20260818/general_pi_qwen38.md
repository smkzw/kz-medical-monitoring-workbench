...
- Runner-managed report path: `runs/conference/medical_monitoring_r4_stage_closure_20260818/general_pi_qwen38.md`. Never write that report path with tools; return the complete report and let the runner persist it.
...
Objective:
独立审计并关闭医学监查 AI 原生重构 R4 风险 Agent 子图阶段；仅在 D01-D10 覆盖、跨域合同、ensemble 1/N、高风险可见性、Query 边界、生命周期、反过拟合和回归证据全部通过后解锁 R5
...
- `kimi` / `kimi-code` / `k3-256k` / effort high

Output schema:
1. `# Conference Participant Output: medical_monitoring_r4_stage_closure_20260818 - general_pi_qwen38`
2. `## Boundary Check`
