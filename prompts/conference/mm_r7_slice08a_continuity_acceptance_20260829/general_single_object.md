...
- Runner-managed report path: `runs/conference/mm_r7_slice08a_continuity_acceptance_20260829/general_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r7_slice08a_continuity_acceptance_20260829_conference_context.md`
- `plans/codex_main_venue_mm_r7_slice08a_continuity_acceptance_20260829.md`
...
Objective:
独立审阅 R7 Slice-08A synthetic/offline continuity 实现与冻结 v0.2 合同的一致性，重点挑战 R2 唯一生命周期、缺行不自动关闭、三模式全量/增量边界、R5/R6摘要语义、SQLite v2到v3 additive迁移、CAS原子发布、确定性与失败关闭；只给 ACCEPT/REVISE 及可复现证据，不修改文件、不运行服务或真实项目。
...
Output schema:
1. `# Conference Participant Output: mm_r7_slice08a_continuity_acceptance_20260829 - general_single_object`
2. `## Boundary Check`
