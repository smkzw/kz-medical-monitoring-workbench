...
- Write exactly one output file: `runs/conference/mw_legacy_reference_reindex/general_opencode_deepseek_flash.md`. The bounded runner persists your final response there; do not create sibling output files.
...
Task:
Independently perform a failure-oriented technical review of unified legacy/new citation indexing. Propose the smallest production-safe data contract, deterministic parser/migration phases, conflict states, editor projection, export algorithm and tests. Explicitly evaluate ranges, multi-citations, bibliography numbering gaps/duplicates, raw-reference preservation, project isolation, idempotent backfill, stale working copies and rollback. Do not look at other participant outputs.

Output schema:
1. `# Conference Participant Output: mw_legacy_reference_reindex - general_opencode_deepseek_flash`
2. `## Boundary Check`
