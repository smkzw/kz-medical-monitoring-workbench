...
- Runner-managed report path: `runs/conference/mm_r7_slice_02_product_api_run_entry_acceptance_20260828/general_pi_antigravity.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r7_slice_02_product_api_run_entry_acceptance_20260828_conference_context.md`
- `plans/codex_main_venue_mm_r7_slice_02_product_api_run_entry_acceptance_20260828.md`
...
Objective:
独立审阅 R7 Slice-02 当前冻结候选：产品 API/运行入口的合同一致性、四层 ExecutionProfile 语义、默认 MTPLX 与显式 DeepSeek、Run replay/conflict、API 连接生命周期、中文错误、测试充分性与受限接受边界；重点寻找 P0-P4 逻辑缺陷，不得修改文件或把隔离实现称为产品挂载
...
Output schema:
1. `# Conference Participant Output: mm_r7_slice_02_product_api_run_entry_acceptance_20260828 - general_pi_antigravity`
2. `## Boundary Check`
