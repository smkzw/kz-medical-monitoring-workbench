...
- Agent/provider/model assigned by Codex: `pi` / `cursor` / `cursor-grok-4.6`
...
- Runner-managed report path: `runs/conference/mm_r7_slice07a_progress_ui_visual_acceptance_20260828/visual_single_object.md`. Never write that report path with tools; return the complete report and let the runner persist it.

Initial read set:
- `context/mm_r7_slice07a_progress_ui_visual_acceptance_20260828_conference_context.md`
- `plans/codex_main_venue_mm_r7_slice07a_progress_ui_visual_acceptance_20260828.md`
...
Objective:
独立审阅修复后的 R7 Slice-07A synthetic 进度界面是否满足中文原生医学监察员的视觉与交互合同；核对最终 v2 失败态层级、ego(lite) DOM/键盘证据、1280 宽屏、R5/R7 同页呈现及最小修复，不修改文件、不运行真实项目、不触碰医学写作，并给出 accept_limited 或 revise。
...
- `codebuddy` / `codebuddy-cli` / `glm-5.3` / effort max

Output schema:
1. `# Conference Participant Output: mm_r7_slice07a_progress_ui_visual_acceptance_20260828 - visual_single_object`
2. `## Boundary Check`
