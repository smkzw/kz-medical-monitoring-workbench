...
- Runner-managed report path: `runs/conference/mm_r7_slice07a_progress_ui_visual_acceptance_20260828/visual_pi_k3_256k.md`. Never write that report path with tools; return the complete report and let the runner persist it.
...
Objective:
独立审阅修复后的 R7 Slice-07A synthetic 进度界面是否满足中文原生医学监察员的视觉与交互合同；核对截图、ego(lite) DOM/键盘证据、七态、真实进度、错误恢复、宽屏层级及最小修复，不修改文件、不运行真实项目、不触碰医学写作，并给出 accept_limited 或 revise。
...
Output schema:
1. `# Conference Participant Output: mm_r7_slice07a_progress_ui_visual_acceptance_20260828 - visual_pi_k3_256k`
2. `## Boundary Check`
