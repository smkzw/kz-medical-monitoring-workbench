...
| B | Pi/OMP `aishuo/cms-model`, reasoning `high` | requested selector, stdout/session `model_change`, durable provider/model | New launches only before 23:30 or from 08:31; no substitute counts as B |
| C | CodeBuddy CLI `hy3`, highest available reasoning | executable/session identity and visible model receipt | Different model fails closed |
...
| E | Pi/OMP `alibaba/qwen3.8-max-preview` with reasoning `xhigh` | requested selector, durable `model_change`, thinking/reasoning receipt and provider/model response identity | New launches only from 22:00 through 06:00; outside-window launch fails closed |

## OMP Integrity

For Tester B, D and E:
...
- Blackout for new Tester B launches: `23:30 <= local time <= 08:30` Beijing,
  including both endpoints.
- To avoid boundary ambiguity, schedule new Tester B passes and follow-ups
  before `23:30` or from `08:31` onward.
...
## Beijing Alibaba Qwen Window

- New Tester E launches and follow-ups are allowed only in the wrapped window
  `22:00 <= local time <= 06:00` Beijing, including both endpoints.
- A schedule outside this strict window is `DEFER`, even if a reason is
  supplied. An already-started pass may finish naturally and must retain its
  original route receipt; it is not silently relaunched under another model.
